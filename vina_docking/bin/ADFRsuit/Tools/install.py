...
# make rmsd script
rmsdScript =  path.join("$ADS_ROOT", pckgsDir, 'ADFR',  'bin', 'rmsd.py')
rmsdlines =  """if test $# -gt 0
then
	exec $python $pyflags %s $@
else
	exec $python $pyflags %s
fi
"""%(rmsdScript, rmsdScript)
rmsdshPath = path.join(bindir, 'rmsd')
f = open(rmsdshPath, "w")
f.writelines(tplLines)
f.write(rmsdlines)
f.close()
os.chmod(rmsdshPath, 509)
...
%s/bin
add  %s/bin to the path environment variable .bashrc:
.bashrc:
export PATH=%s/bin:$PATH

"""%(ads_root, ads_root, ads_root)
