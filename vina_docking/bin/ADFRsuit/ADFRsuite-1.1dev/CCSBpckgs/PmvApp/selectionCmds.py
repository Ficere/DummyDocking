...
class MVSetSelectionLevel(MVCommand):
    ...
    def undoCmdBefore(self, Klass, KlassSet=None):
        if (
            not (
                self.app().selectionLevel in [Molecule, Protein]
                and Klass in [Molecule, Protein]
            )
            and self.app().selectionLevel != Klass
        ):
            if not self.app().activeSelection.empty():
                ...
                return (
                    [
                        (
                            self.app().select,
                            (self.app().activeSelection.atoms.copy(),),
                            {"only": 1},
                        ),
                        (
                            self,
                            (self.app().selectionLevel,),
                            {
                                "KlassSet": None,
                            },
                        ),
                ...
                return (
                    [(self, (self.app().selectionLevel,), {"KlassSet": None})],
                    self.name,
                )
    ...
    def doit(self, Klass, KlassSet=None):
        ...
        event = selectionFeedbackEvent(
            objects=self.app().activeSelection.get(), highlightSelection=False
        )
...
class MVSelectCommand(MVCommand):  # , MVAtomICOM):
    ...
    def onAddCmdToApp(self):
        if not hasattr(self.app(), "setSelectionLevel"):
            self.app().lazyLoad(
                "selectionCmds", commands=["setSelectionLevel"], package="PmvApp"
            )

    def checkArguments(
        self, molSel, negate=False, xor=False, intersect=False, only=False
    ):
        """
        Required Arguments:\n
    ...
    def doit(
        self, inSelectionSet, negate=False, xor=False, intersect=False, only=False
    ):
        ...
        if only:
            self.app().pushUndoCmd(
                self.app().select, (activeSelectionSet.copy(),), {"only": True}
            )
            ...
            for inSelection in inSelectionSet:
                ...
                if negate:  # nodes will be unselected
                    if curSel is not None:
                        ...
                        self.app().pushUndoCmd(
                            self.app().select, (inSelection,), {"negate": False}
                        )
                ...
                ##         self.app().pushUndoCmd( self.app().select, (exor,), {})
                    ...
                    self.app().pushUndoCmd(
                        self.app().select, (inSelection,), {"negate": True}
                    )
    ...
    def clear(self):
        ...
        event = SelectionEvent(
            app.activeSelection, new=[], old=setOff, setOn=[], setOff=setOff
        )
...
class MVSaveSetCommand(MVCommand):
    ...
    def onAddCmdToViewer(self):
        ...
        self.app().eventHandler.registerListener(
            DeleteAtomsEvent, self.handleDeleteAtomsEvents
        )
    ...
    def checkArguments(self, nodes, name, comments="No description"):
        ...
        if name in self.app().sets.keys():
            ...
            self.app().warningMsg(
                "set name %s already used\nrenaming it %s" % (name, newname)
            )
            name = newname
...
class MVSelectSetCommand(MVCommand):
        ...
        for k in self.app().sets.keys():
            ...
            if mol in nodes.top.uniq():
                if len(nodes.top.uniq()) == 1:
                    ...
                    newComments = (
                        self.app().sets[k].comments
                        + " the elements of this set belonging to %s have been deleted with the molecule"
                        % mol.name
                    )
                    del self.app().sets[k]
    ...
    def undoCmdBefore(self, name, negate=False, only=False):
        ...
        if len(self.app().activeSelection) == 0:
            return (
                [(self.app().clearSelection, (), {})],
                self.app().clearSelection.name,
            )
        else:
            return (
                [(self.app().select, (self.app().activeSelection.atoms.copy(),), {})],
    ...
    def checkArguments(self, setNames, negate=False, only=False):
        ...
        if isinstance(setNames, str):
            ...
            if not isinstance(setNames, (list, tuple)):
                raise RuntimeError(
                    "%s, setNames should be a string or a list of strings" % self.name
                )
            for name in setNames:
                assert isinstance(name, str)
        assert negate in [1, 0, True, False]
...
class MVSelectFromStringCommand(MVCommand):
    ...
    def doit(
        ...
        selitem, selMsg = StringSelector().select(
            nodes.top.uniq(), selectionString, app.sets, caseSensitive, escapeCharacters
        )
        ...
        if selitem and len(selitem) > 0:
            ...
            #                app.setSelectionLevel(selitem[0].__class__, busyIdle=0, log=0)
            app.select(
                selitem, negate=negate, xor=xor, intersect=intersect, setupUndo=False
            )
            # if not negate:
...
class MVSelectSphericalRegion(MVCommand):
    ...
    def onAddCmdToViewer(self):
        ...
        self.masterGeom = Geom("selectInSphereGeoms", shape=(0, 0), protected=True)

        self.selSph = Spheres(
            name="SelSphReg_selSph",
            materials=((1.0, 1.0, 0),),
            shape=(0, 3),
            radii=5.0,
            quality=15,
        ...
        event1 = AddGeometryEvent(self.masterGeom, parent=miscGeom)
        self.app().eventHandler.dispatchEvent(event1)
        event2 = AddGeometryEvent(self.selSph, redo=0, parent=self.masterGeom)
        self.app().eventHandler.dispatchEvent(event2)
        event3 = AddGeometryEvent(self.cenCross, redo=0, parent=self.masterGeom)
    ...
    def undoCmdBefore(self, *args, **kw):
        select = self.app().select
        oldselection = self.app().activeSelection.atoms.copy()
        return ([(self.app().select, (oldselection,), {"only": True})], self.name)
...
class SelectNoWaterHeteroAtomsCommand(MVSelectFromStringCommand, MVSelectCommand):
    ...
    def checkArguments(
        self, nodes=None, negate=False, only=False, xor=False, intersect=False
    ):
        """
        Required Arguments:\n
            nodes --- can be a string, a TreeNode or a treeNodeSet \n
    ...
    def doit(self, nodes, negate=False, only=False, xor=False, intersect=False):
        ...
        het = AtomSet(
            [
                a
                for a in allAtoms
                if a.hetatm and a.parent.type != "WAT" and a.parent.type != "HOH"
            ]
        )
        ...
        # the following event:
        event = selectionFeedbackEvent(
            objects=self.app().activeSelection, highlightSelection=True
        )
        self.app().eventHandler.dispatchEvent(event)
