...
of view when a new molecule is loaded""")
    ...
    def multiGoto(self, mol, idx, item=None):
        """got to  a molecule or a conformation for MultiMolecules and MultiConformers molecules
        atIndex is specified by dashboard in order to put the molecule back in dashboard"""
        ...
        if mol._multi=='molecules':
            ...
            if item:
                ...
                atIndex = 0
                wasSelected = False
            self.deleteMolecule(mol, setupUndo=False)
            self._addMolecule(mol)
            ...
            if mol._renderingBits & mol._CPK and not hasattr(self.displayCPK, 'loadCommand'):
                self.displayCPK(mol, setupUndo=False)
            if mol._renderingBits & mol._SB:
                self.displaySB(mol, setupUndo=False)
            if mol._renderingBits & mol._SURFACE:
                surfName = '%s_surface'%mol._basename
                self.computeMSMS.guiCall(mol, surfName=surfName)
                self.displayMSMS.guiCall(mol, surfNames=[surfName])
            if mol._renderingBits & mol._CARTOON:
                self.computeCartoon(mol)
                self.displayCartoon(mol)
            if mol._labelingBits & mol._ATOM_LABEL:
                self.labelAtoms(mol)
            if mol._labelingBits & mol._RESIDUE_LABEL:
                self.labelResidues(mol)
            ...
            for mol in mols:
                ...
                mol.setBondorder(mol._ag._bondOrder)
                if mol.geomContainer.geoms.has_key('aromaticSpheres'):
                    bonds = mol.select().getBonds()
                    if len(bonds[4]):
                        self.displaySB.updateAromaticArcs(mol, bonds[4])
                ...
                if hasattr(mol, "_msmsData"):
                    for name in mol._msmsData['msms'].keys():
                        if mol._msmsData['msms'][name][idx] is None:                        
                            self.computeMSMS(mol, surfName=name,
                ...
                self.eventHandler.dispatchEvent(event)
                self.gui().viewer.selectionEventHandler()
...
provided by obj
    ...
    def addMolecule(self, newmol, group=None, filename=None):
        ...
        """
        self._addMolecule(newmol, group, filename)
        self.applyDefaultCommands(newmol, "Molecule")
        event = AfterAddMoleculeEvent(molecule=newmol)
        self.eventHandler.dispatchEvent(event)
        
    def _addMolecule(self, newmol, group=None, filename=None):
        """
        Add a molecule to the application 
        """
...
"""
    ...
    def focusScene(self, obj=None, viewer=None, duration=0.5, padding=10):
        ...
        if isinstance(obj, Geom):
            coords = obj.vertexSet.vertices.array
        else:
            if obj is not None:
                if isinstance(obj, SelectionSet):
                    sel = obj
        ...
        if len(coords):
            ...
            if viewer.currentCamera.ssao: # ambient occlusion is on
                ...
                ssaoNear = None
            viewer.Reset_cb()
            viewer.Normalize_cb(padding=padding)
            viewer.Center_cb()
            ...
            mg.interpolate(duration, rootObj, transf=[{rootObj:transf1}, {rootObj:transf2}],
                           camattr=[ca1, ca2])
