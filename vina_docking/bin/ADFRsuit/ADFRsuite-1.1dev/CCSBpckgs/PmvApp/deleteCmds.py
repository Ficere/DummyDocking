...
class DeleteMolecule(MVCommand):
    ...
    def updateUndoRedoCmdStack(self, molecule, command):
        ...
        for i, cmdlist in enumerate(command.cmdStack):
            ...
            for j in xrange(
                len(cmdlist[0]) - 1, -1, -1
            ):  # loop backwards over the list of commands in undo/redo action
                cmd, args, kw = cmdlist[0][j]
                if len(args):
                    newargs0 = []  # this list will contain updated atom selections
                    newargs = (
                        []
                    )  # this list will contain other arguments that are not atomsets or molecules
                    for arg in args:
                        if (
                            isinstance(arg, list)
                            and len(arg)
                            and isinstance(arg[0], Molecule)
                        ):
                            mollist = []
                            for mol in arg:
                                if mol != molecule:
                            ...
                            if arg.getAtomGroup() != molecule._ag:
                                newargs0.append(arg)
...
class DeleteAtoms(MVCommand):
    ...
    def onAddCmdToApp(self):
        if not self.app().commands.has_key("deleteMolecule"):
            self.app().lazyLoad(
                "deleteCmds", commands=["deleteMolecule"], package="PmvApp"
            )
        self.app().lazyLoad(
            "selectionCmds", commands=["select", "clearSelection"], package="PmvApp"
        )
    ...
    def updateUndoRedoCmdStack(self, oldAg, mol, indMap, command):
        ...
        for i, cmdlist in enumerate(command.cmdStack):
            ...
            # ( [ (cmd, (args,) {kw}), (cmd, (args,) {kw}), ... ], "undo/redo name" )
            # import pdb; pdb.set_trace()
            cmdName = cmdlist[1].split(" ")[
                0
            ]  # name of the undo command without the string representing its arguments
            newName = ""
            ncmds = len(cmdlist[0])
            for j in xrange(
                len(cmdlist[0]) - 1, -1, -1
                ...
                if len(args):
                    ...
                    newargs = (
                        []
                    )  # this list will contain arguments that are not Molecules ar atom selections
                    for arg in args:
