...
from MolKit2.molecule import (
    Molecule,
    Atom,
    MoleculeSet,
    MOLECULE_RESIDUE_LABEL,
    MOLECULE_ATOM_LABEL,
)
...
class LabelBase(DisplayCommand):
    ...
    _argNames = ["label", "propName", "font", "fontSize", "formatString"]
    ...
    def _initializeMolForCmd(self, mol):
        ...
        if mol._colors.get(self._atomSetName, None) is None:
            ...
            mol._colors[self._atomSetName] = numpy.array([[1.0, 1.0, 1.0, 1.0]], "f")

        if mol._ag.getData("colorsIndices_%s" % self._atomSetName) is None:
            mol._ag.setData(
                "colorsIndices_%s" % self._atomSetName, numpy.zeros(len(mol._ag), "i")
            )
        ...
        if mol._renderingProp.get(self._atomSetName, None) is None:
            mol._renderingProp[self._atomSetName] = {
                ...
                "backfacecolor": numpy.array([[0.0, 0.0, 0.0]], "f"),
        ...
        if not gc.geoms.has_key(self._atomSetName):
            g = GlfLabels(
                ...
                fontTranslation=(
                    ...
                    0,
                    0.1,
                ),  # so the label is less hidden by the structure
    ...
    def updateModelGlobals(
        ...
        **kw
        ...
        if fontSize is not None:
            ...
            mol._renderingProp[self._atomSetName]["fontSize"] = fontSize
        ...
        if frontrendering is not None:
            ...
            mol._renderingProp[self._atomSetName][
                "frontrendering"
            ] = self._renderToDejaVu[frontrendering]
        if frontrendering is not None:
            assert frontrendering in self._renderToDejaVu.keys()
            mol._renderingProp[self._atomSetName][
                "frontrendering"
        ...
        if backrendering is not None:
            ...
            mol._renderingProp[self._atomSetName][
                "backrendering"
            ] = self._renderToDejaVu[backrendering]
...
class LabelAtoms(LabelBase):
    def __init__(self):
        self.fontSize = (0.45, 0.45, 0.01)  # hold current fontSize
    ...
    def updateModel(self, atoms, labels=None, propName="name", **kw):
        ...
        if propName is not None and labels is None:
            ...
            if newPropLabels is None:
                msg = "%s: specified nodes do not have property: %s" % (
                    self.name,
                    propName,
                )
                ...
                if propName == "hbType":
                    newPropLabels = self.getHBLabels(
                        newPropLabels, labelGeom.labels.dtype
                    )

            formatString = kw.get("formatString", None)
            if formatString is None:
                mol._renderingProp[self._atomSetName]["formatString"]
            if formatString is None:
                labels = [str(item) for item in newPropLabels]
            else:
                labels = [formatString % item for item in newPropLabels]
    ...
    def refreshDisplay(self, mol):
        ...
        if len(atoms) == 0:  # nothing is diplayed as CPK anymore for this molecule
            ...
            geom.Set(fontScales=globProp["fontSize"], redo=0)
            ...
            geom.Set(
                vertices=atoms.getCoords(),  # faces=[[x] for x in indices],
                labels=labels[indices],
                labelTranslation=labOff[indices],
                visible=1,
                tagModified=False,
                materials=colors[indices],
                inheritLineWidth=False,
                lineWidth=globProp["linewidth"],
                inheritPointWidth=False,
                pointWidth=globProp["pointwidth"],
