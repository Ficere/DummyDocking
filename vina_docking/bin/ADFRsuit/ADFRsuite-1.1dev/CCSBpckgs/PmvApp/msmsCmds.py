...
class ComputeMSMS(MVCommand):  # , MVAtomICOM):
    ...
    def checkArguments(self, nodes, **kw):
        for name in kw.keys():
            if name not in self._argNames:
                raise RuntimeError(
                    "%s: unrecognized keyword argument '%s', valid names are %s"
    ...
    def onAddCmdToApp(self):
        ...
        app = self.app()
        app.eventHandler.registerListener(DeleteAtomsEvent, self.handleDeleteAtoms)
        ...
        **kw
        ...
        if mol._msmsData["msms"].has_key(surfName):
            if mol._msmsData["msms"][surfName][mol._ag.getACSIndex()] is not None:
                # find existing
                # we already have a surface, check if paramters changes
                recompute, kw = self.recomputeMSMS(
                    mol,
                    molSel,
                    surfName,
                    probeRadius,
                ...
                if not recompute:
                    if len(sel) != len(mol._msmsData["atoms"][surfName]) or len(
                        sel & mol._msmsData["atoms"][surfName]
                    ) < len(sel):
            ...
            if cavities is None:
                cavities = (
                    mol.app().userpref["Compute cavities by default"]["value"] == "yes"
                )
        ...
        if not mol._msmsData["msms"].has_key(surfName):
            ...
            if mol._renderingProp.get("msms", None) is None:
                ...
                mol._renderingProp["msms"][surfName] = {
                    ...
                    "culling": "none",
                    "shading": "smooth",
                    "backfaceColor": [1.0, 1.0, 1.0, 1.0],
                    "components": "all",  # of list of component indices
                }
                mol._renderingProp["msms"][surfName]["compVisible"] = [
                    True
                ] * srf.sesr.nb
            ...
            geomC.addGeom(
                g,
                displayCmd=self.app().displayMSMS,
            ...
            mol._ag.setData(
                "colorsIndices_msms_%s" % surfName,
    ...
    def pickedVerticesToAtoms(self, geom, vertInd):
        ...
        return allAtInds[
            numpy.unique(
                geomC.geoms["msms_" + surfName]._vertexToAtomInd[list(vertInd)]
            )
        ]
...
class DisplayMSMS(DisplayIndexedGeomCommand):
    ...
    def doit(self, molSel, **kw):
        ...
        self.app().labelAtoms.refreshDisplay(
            mol
        )  # to make the labels be in front of MSMS
        self.app().labelResidues.refreshDisplay(
            mol
        )  # to make the labels be in front of MSMS

    def updateModelGlobals(
        self,
        mol,
        name,
        propDict,
        ...
        **kw
        ...
        DisplayIndexedGeomCommand.updateModelGlobals(
            self,
            mol,
            propDict,
            frontRendering=frontRendering,
            backRendering=backRendering,
            linewidth=linewidth,
        ...
        for name in names:
            ...
            if len(atoms) == 0:  # nothing is displayed as CPK anymore for this molecule
                ...
                if len(atomindices) == 0:
                    ...
                    if (
                        len(globProp["compVisible"]) < len(compIndices)
                        and len(globProp["compVisible"]) == 1
                    ):
                    ...
                    for compNum in compIndices:
                        if (
                            globProp["compVisible"]
                            and not globProp["compVisible"][compNum]
                        ):
                            continue
                        vfc, vic, fc = srf.getTriangles(
                            atomindices,
                            component=compNum,
                            selnum=1,
