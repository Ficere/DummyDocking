...
class Sheet2D:
    ...
    def compute(
        self, coords, isHelix, nbrib=2, nbchords=10, width=1.5, offset=1.2, off_c=0.5
    ):
        """ """
        self.nrib = nbrib
        self.coords = coords
        ...
        self.smooth = Ribbon.ribbon2D(
            nbrib, width, nbchords, offset, natoms, coords, isHelix, off_c
        )
        self.oldsmooth = self.smooth
        self.addFirstLastPoints()
        self.verts2D_flat = numpy.array(numpy.reshape(self.smooth, (-1, 4))[:, :3])
    ...
    def addFirstLastPoints(self):
        ...
        self.smooth = numpy.concatenate(
            (self.smooth[:, :-2, :], ar2, self.smooth[:, -1:, :]), 1
        )
    ...
    def computeFaces2D(self):
        ...
        faces2D = numpy.array(
            [(x, x + n, x + n + 1, x + 1) for x in range(n - 1)]
        ).astype("i")
        return faces2D

    def computeBinormals(self):
        f = numpy.array(self.faces2D[:, :3])
        binorm = TriangleNormals(self.verts2D_flat, f, "PER_VERTEX")
...
class ExtrudeObject:
    ...
    def __init__(
        self, path3D, matrix, shape, cap1=0, cap2=0, arrow=0, larrow=3, warrow=2
    ):
        """
        ...
        self.cap2 = cap2
        if self.arrow:
            self.cap2 = 0  # no end cap if arrow
        self.norms = matrix[:, 0]
    ...
    def getextrudeVertices(self):
        ...
        if self.cap1:
            f, g = f + 1, g + 1  # add a section and a middle point.
        if self.cap2:
            f, g = f + 1, g + 1
        if self.arrow:
            f = f + 3  # add 3 sections:
        ...
        for i in range(length):
            pts[i * ls : i * ls + ls] = numpy.dot(
                self.shape.contpts, self.matrix[i]
            ).astype("f")
            newNorm = numpy.dot(self.shape.contnorm, self.matrix[i]).astype("f")
    ...
    def getFaces(self):
        ...
        if self.shape.vertDup == 1:
            ...
            if not self.arrow:
                faces = self.buildFaces(
                    faces=faces, start=0, end=len(self.path3D) - 1, ls=ls, dup=1
                )
            else:
                ...
                faces = self.buildFaces(
                    faces=faces, start=0, end=lp - (self.larrow + 1), ls=ls, dup=1
                )
        ...
        # not duplicate vertices; 1 normal per vertex
            ...
            if not self.arrow:
                faces = self.buildFaces(
                    faces=faces, start=0, end=len(self.path3D) - 1, ls=ls, dup=0
                )
            else:
                # ARROWS: 3*ls vertices have been added but only 2 faces.
                lp = len(self.path3D)
                # 1- BLOC1: cf drawing above.
                faces = self.buildFaces(
                    faces=faces, start=0, end=lp - (self.larrow + 1), ls=ls, dup=0
                )
    ...
    def buildFaces(self, faces, start, end, ls, dup):
        ...
        for i in xrange(start, end):
            ...
            # Build the last face.
            faces.append(
                (i * ls, (i + 1) * ls, (i + 1) * ls + (ls - 1), (i * ls) + (ls - 1))
            )
        return faces
    ...
    def setStripProperty(self, properties, propName):
        ...
        if self.cap2:
            c = c + nfaces
        for i in range(-c, 0):
            self.colors[i] = (1.0, 1.0, 1.0)  # color caps white
            self.opacities[i] = 1.0  # color caps white
        self.properties = getattr(self, propName)
        if self.arrow:
...
class ExtrudeSSElt(ExtrudeObject):
    ...
    def __init__(
        ...
        matrix = self.ssElt.sheet2D.matrixTransfo[self.fromForStart : self.toForEnd]
    ...
    def getResIndexFromPts(self, respts):
        ...
        if respts < (self.chords / 2 + 1):
            # first residue
            resIndex = 0
        elif respts > ((self.lengthPath - 1) - (self.chords + self.chords / 2)):
            # last residue
    ...
    def setResProperties(self, propVect, propName, ResSet):
        ...
        ExtrudeObject.setResidueProperties(self, properties, propName, firstLastIndices)
    ...
    def getResIndexFromExtrudeVertex(self, vertex):
        ...
        elif pickedSection > ((self.lengthPath - 1) - (self.chords + self.chords / 2)):
            resIndex = self.lastResIndex
    ...
    for res in chain.DNARes[1:]:
        ...
        if NA_type in ["A", "G"]:
            ...
            if not missingAts:
                N9 = listeCoord[
                    0
                ]  # numpy.array(atoms.objectsFromString('N9.*')[0].coords)
                C8 = listeCoord[
                    1
                ]  # numpy.array(atoms.objectsFromString('C8.*')[0].coords)
                C4 = listeCoord[
                    2
                ]  # numpy.array(atoms.objectsFromString('C4.*')[0].coords)
                N9_C8 = C8 - N9
                N9_C4 = C4 - N9
                normal = numpy.array(crossProduct(N9_C8, N9_C4, normal=True))
        else:
            ...
            if not missingAts:
                N1 = listeCoord[
                    0
                ]  # numpy.array(atoms.objectsFromString('N1.*')[0].coords)
                C2 = listeCoord[
                    1
                ]  # numpy.array(atoms.objectsFromString('C2.*')[0].coords)
                C6 = listeCoord[
                    2
                ]  # numpy.array(atoms.objectsFromString('C6.*')[0].coords)
                N1_C2 = C2 - N1
                N1_C6 = C6 - N1
                normal = numpy.array(crossProduct(N1_C2, N1_C6, normal=True))
