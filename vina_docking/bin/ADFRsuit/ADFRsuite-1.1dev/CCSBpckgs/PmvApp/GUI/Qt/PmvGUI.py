...
from MolKit2.molecule import Atom, Molecule, MoleculeSet, MultiMolecule, Residue, Chain
...
try:
    ...
    # runtime error occurs is QT_API is set to a bad value
...
class LabeledLineEdit(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        layout.addWidget(QtGui.QLabel('goto:'))
...
class PmvViewer(Viewer, QtGui.QWidget):
    ...
    def __init__(self, pmv, master=None, nogui=0, 
        ...
        # list of function to call upon picking. When atoms are picked the funtions in the
        # list are called with a Selection object for the atoms and an operation that can
        # be 'add' or 'remove'.
        # for bonds the function is passes a list [(mol, at1, at2)] and the operation
        self.pickedAtomsFuncs = [] # list of function to be called when atoms are picked
        self.pickedBondsFuncs = [] # list of function to be called when bonds are picked
        self.pickedAtomsFuncs.append(self.updateAtomSelection)
    ...
    def getFilename(self, filenames=None):
        filenames, filter = QtGui.QFileDialog.getOpenFileNames(
            ...
            #self.tr("PDB Files (*.pdb);; PDBQT Files (*.pdbqt);; MOL2 Files (*.mol2);; SDF Files (*.sdf);; CIF Files (*.cif *.cif.gz);;;; MMTF Files (*.mmtf);; Session Files (*.psf);; Docking Results Object Files(*.dro);; All files (*) "))
            self.tr("molecule Files (*.pdb *.mol2 *.sdf *.cif *.cif.gz *.mmtf *.pdbqt);; PDB Files (*.pdb);; MOL2 Files (*.mol2);; SDF Files (*.sdf);; CIF Files (*.cif *.cif.gz);; MMTF Files (*.mmtf);; Session Files (*.psf);; Docking Results Object Files(*.dro);; All files (*) "))
            ...
            if len(fi.suffix())==0:
                for e in extensions:
                    if filtr.find(e) > 0:
                        ...
                        break
            print filename
            self.pmv.saveSelectionAsMolecule(obj, filename)
    ...
    def setPickingMode(self, mode):
        assert mode in ['atoms', 'bonds', 'both']
        if mode=='atoms':
    ...
    def processPicking(self, pick):
        ...
        if len(pick.hits):
            if pick.type=='vertices':
                ...
                if atoms:
                    for f in self.pickedAtomsFuncs:
                        f(atoms, pick.operation)
            elif pick.type=='parts':
                bonds = self.findPickedBonds(pick)
                if bonds:
                    for f in self.pickedBondsFuncs:
                        f(bonds, pick.operation)
        else:
            if self.emptyPickFunc:
                self.emptyPickFunc()

    def updateAtomSelection(self, atoms, operation):
        if len(atoms):
            if operation=='add': negate = False
            else: negate = True
...
"""
    ...
    def findPickedBonds(self, pick):
        ...
        for o, val in pick.hits.items(): #loop over geometries
            ...
            for instvert in val:
                ...
                gc = o.mol().geomContainer
                if gc.geomPickToBonds.has_key(o.name):
                    func = g.geomPickToBonds[o.name]
                    if func: allbonds = allbonds + func(o, primInd)
                else:
                    l = []
                    at1, at2 = o.mol()._ag[o.faceSet.faces.array[primInd]]
                    allbonds.append((o.mol(), at1, at2))
    ...
    def buildMenuForCamera(self, camera, parent=None):
        ...
        def askCamColor():
            ...
            w.setAttribute(QtCore.Qt.WA_DeleteOnClose) # this fixes a bug in MacOSx native color dialog, when it adds another row of "OK-Cancel" buttons each time the dialog opens in modal mode.
            value = w.exec_()
            if value==0:
...
"""
    ...
    def gotoMultiMolEntry(self, item, menu):
        menu.setVisible(False)
        mol = item._pmvObject
        text = self._lineedw.lineEdit.text()
        n = None
        if text[0]=='#':
            try:
                n = int(text[1:])-1
            ...
            n = mol.getMoleculeIndexByName(text)
        if n is not None:
            self.pmv.gui().objTree.multiGoto(mol, n, item)
    ...
    def buildMenuForObject(self, objItems, level=None, parent=None, parentMenu=None,
        ...
        if item0: # could be None which righ click in 3d View
            ...
            if root is item0: # we are at the top
                if isinstance(item0._pmvObject, MultiMolecule):
                    self._lineedw = linew = LabeledLineEdit(menu)
                    cb = CallbackFunction(self.gotoMultiMolEntry, item0, menu)
                    #linew.lineEdit.textChanged.connect(cb)
                    linew.lineEdit.returnPressed.connect(cb)
                    lineAction = QtGui.QWidgetAction(menu)
                    lineAction.setDefaultWidget(linew)
    ...
    def customizeRepr(self, objItems, selSet):
        ...
        if not self.customizeReprDialog:
            ...
            def _onClose():
                if parent and hasattr(parent, "_openDialogs"):
                    if dialog in parent._openDialogs:
                        parent._openDialogs.remove(dialog)
                else:
                    if hasattr(gui, "_openDialogs") and dialog in gui._openDialogs:
                        gui._openDialogs.remove(dialog)
            dialog.onClose.connect(_onClose)
        else:
        ...
        if parent and hasattr(parent, "_openDialogs"):
            if self.customizeReprDialog not in parent._openDialogs:
                parent._openDialogs.append(self.customizeReprDialog)
        else:
            if hasattr(gui, "_openDialogs") and self.customizeReprDialog not in gui._openDialogs:
                gui._openDialogs.append(self.customizeReprDialog)
        ...
        if mol._multi=='molecules':
            if mol._hasIndex is False:
                ...
                item.setText(0, "%s %s %d/%d"%(
                    mol._basename, mol.componentNames[mol.currentMoleculeIndex], mol.currentMoleculeIndex+1, len(mol.index)))
