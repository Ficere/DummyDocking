...
class BuildBondsByDistance(MVCommand):  # , MVAtomICOM):
    ...
    def undoCmdAfter(self, *args, **kw):
        if len(self.createdBonds):
            return (
                [(self.app().removeBonds, (self.createdBonds,), {})],
...
class AddBondsGUICommand(MVCommand):  # , MVAtomICOM):
    ...
    def onAddCmdToApp(self):
        ...
        self.app().lazyLoad(
            "bondsCmds", commands=["addBonds", "removeBondsGC"], package="PmvApp"
        )

        self.masterGeom = Geom("addBondsGeom", shape=(0, 0), pickable=0, protected=True)
        self.masterGeom.isScalable = 0
        self.spheres = Spheres(
            name="addBondsSpheres",
            shape=(0, 3),
            inheritMaterial=0,
            radii=0.2,
            quality=15,
    ...
    def doit(self, ats):
        if len(ats) > 2:
            ...
            for i in range(0, lenAts, 2):
                ...
                at2 = atSet[i + 1]
                self.app().addBonds(
                    [(at1, at2)], bondOrder=[1], origin="UserDefined", log=0
                )
        self.update(True)
    ...
    def doit(self, bondAtomPairs, bondOrder, origin):
        ...
        for atoms, bo in zip(bondAtomPairs, bondOrder):
            ...
            if atom1.top != atom2.top:
                msg = "intermolecular bond between %s and %s disallowed" % (
                    atom1.full_name(),
                    atom2.full_name(),
                )
                self.app().warningMsg(msg)
            ...
            if len(bnds):
                msg = "bond between %s and %s already exists" % (
                    atom1.full_name(),
                    atom2.full_name(),
                )
                self.app().warningMsg(msg)
