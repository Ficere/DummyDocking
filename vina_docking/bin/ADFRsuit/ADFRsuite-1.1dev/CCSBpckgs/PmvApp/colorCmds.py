...
class ColorCommandBase(MVCommand):
    ...
    """
    _argNames = ['geomsToColor', 'carbonsOnly', 'colorLabels']
    ...
    def checkArguments(self, *args, **kw):
        for name in kw.keys():
            ...
            if name in ['carbonsOnly', 'colorLabels'] and not (kw[name] or not kw[name]):
                raise ValueError("%s: invalid value for argument '%s', valid names are True or False"%(
                    self.name, name))
    ...
    def doit(self, atoms, geomsToColor=['all',], carbonsOnly=False, colorLabels=False):
