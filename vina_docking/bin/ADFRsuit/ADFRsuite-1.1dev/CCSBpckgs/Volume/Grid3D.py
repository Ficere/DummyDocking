...
class Grid3D:
    ...
    def transpose(self, copy=False):
        ...
        """
        art = numpy.ascontiguousarray(numpy.transpose(self.data), self.data.dtype)
        ...
        if copy is False:
            ...
            newgrid = self.__class__(art, self.origin, self.stepSize[::-1], self.header)
        return newgrid
    ...
    def get2DOrthoSlice(self, axis, sliceNum):
        ...
        if axis == "x":
            ...
            raise ValueError("axis can only be 'x', 'y', or 'z', got %s" % str(axis))
        ...
        if sliceNum > dims[axisInd]:
            raise ValueError(
                "sliceNum for axis %s too large, max %d got %d"
...
ArrayTypeToGrid = {
    numpy.float64: Grid3DD,
    numpy.float: Grid3DD,
    numpy.float32: Grid3DF,
    numpy.float16: Grid3DF,
