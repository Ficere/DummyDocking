...
def writePDB(coords, potentials, atypes, filename):
    ...
    for n, (xyz, pot, aty) in enumerate(zip(coords, potentials, atypes)):
        x, y, z = xyz
        f.write(
            "ATOM%7d%4s   FP     1    %8.3f%8.3f%8.3f  1.0%8.3f    0.001 %s\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="matchPocket",
        usage="usage: %(prog)s --receptor or --maps [options] filename",
        version="%prog 0.1",
    )
    parser.add_argument(
        "-p",
