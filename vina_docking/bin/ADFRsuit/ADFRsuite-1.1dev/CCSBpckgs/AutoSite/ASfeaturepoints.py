...
class featurePts:
    ...
    def getFeaturePts(self):
        ...
        # import pdb
        # pdb.set_trace()
        self.cfp, self.cfpp, self.cBres = self.recPharmacophores(
            self.cc,
            self.cp,
            [
                "NA",
        ...
        if nbc > 14 * self.scale:
            ...
            cfpK = Kmeans(
                self.cc, int(round(nbc / (14.3 * self.scale))), returnCentroids=True
            )
        ...
        self.ofp, self.ofpp, self.oBres = self.recPharmacophores(
            self.oc, self.op, ["HD"], 2.5
        )

        self.hfp, self.hfpp, self.hBres = self.recPharmacophores(
            self.hc, self.hp, ["NA", "OA", "SA"], 2.5
        )
            ...
            if nb:
                for ind, d2 in zip(results[:nb], dist2[:nb]):
                    if recAtoms.getData("AD_element")[ind] in recat:
                        resname = "%s:%s%d" % (
                            recAtoms.getChids()[ind],
                            recAtoms.getResnames()[ind],
                            recAtoms.getResnums()[ind],
                        )
