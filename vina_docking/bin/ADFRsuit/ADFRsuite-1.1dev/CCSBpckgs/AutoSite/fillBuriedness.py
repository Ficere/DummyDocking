...
class Buriedness:
    ...
    def inflatePocketandBuriedness(self, padding=1.0, spacing=1.0, origin=None):
        ...
        set1LargeInd, set1LargeCoords = self.spheres2Grid(
            centersL, numpy.array(radiiL) + padding, mini, spacing
        )
        ...
        # import pdb;pdb.set_trace()
        set2Ind, set2Coords = self.spheres2Grid(
            centersR, numpy.array(radiiR) + 1, mini, spacing
        )
        for inds in set2Ind:
            if shellDict.has_key(inds):
                del shellDict[inds]
    ...
    def NumericalBurriedness(self, padding=1.0, spacing=0.5, origin=None):
        ...
        # get (i,j,k) for enlarged first set of sphere
        set1LargeInd, set1LargeCoords = self.spheres2Grid(
            centersL, numpy.array(radiiL) + padding, mini - 2 * padding, spacing
        )

        # remove indices for first set of sphere with original radii
        set1Ind, set1Coords = self.spheres2Grid(
            centersL, radiiL, mini - 2 * padding, spacing
        )
        shellDict = {}
        for ind, coords in zip(set1LargeInd, set1LargeCoords):
            shellDict[ind] = coords
        ...
        # import pdb;pdb.set_trace()
        set2Ind, set2Coords = self.spheres2Grid(
            centersR, numpy.array(radiiR) + 1, mini - 2 * padding, spacing
        )
        removedCoords = []
        for inds in set2Ind:
            if shellDict.has_key(inds):
