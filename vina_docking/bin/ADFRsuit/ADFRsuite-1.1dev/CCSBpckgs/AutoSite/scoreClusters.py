...
def scoreClusters(rec, dcl, gc, inflate=False, pepScore=False):
    ...
    for clinds in dcl._clusters:
        ...
        if gridSpacing == 0:
            gridSpacing = sqrt(
                (clc[0][0] - clc[1][0]) * (clc[0][0] - clc[1][0])
                + (clc[0][1] - clc[1][1]) * (clc[0][1] - clc[1][1])
                + (clc[0][2] - clc[1][2]) * (clc[0][2] - clc[1][2])
            ) / sqrt(
                (cli[0][0] - cli[1][0]) * (cli[0][0] - cli[1][0])
                + (cli[0][1] - cli[1][1]) * (cli[0][1] - cli[1][1])
                + (cli[0][2] - cli[1][2]) * (cli[0][2] - cli[1][2])
            )
        ...
        if len(cli) == 1:
            metric = Rg = fburied = 0.0
        else:
            # import pdb;pdb.set_trace()
            dmetric = Buriedness(clc, cla, coords.tolist(), radii, origin=[ox, oy, oz])
            if inflate == True:
                fburied, inflateIndices = dmetric.inflatePocketandBuriedness(
                    origin=[ox, oy, oz], spacing=1.0
                )
                for ind, coord in inflateIndices.iteritems():
                    # import pdb;pdb.set_trace()
                    if (
                        ind[0] >= odata.shape[0]
                        or ind[1] >= odata.shape[1]
                        or ind[2] >= odata.shape[2]
                    ):
                        continue
                    ...
                    potential = 0.0
                    potential = min(
                        hdata[ind[0]][ind[1]][ind[2]],
                        cdata[ind[0]][ind[1]][ind[2]],
                        odata[ind[0]][ind[1]][ind[2]],
                        0.0,
                    )
        ...
        clProp.append(
            [numpy.sum(clp), len(clc), numpy.sum(clp) / len(clc), Rg, fburied, metric]
        )
