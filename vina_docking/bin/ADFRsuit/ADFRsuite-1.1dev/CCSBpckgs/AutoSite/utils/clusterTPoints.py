    ...
    def kmeansCluster(self):
        ...
        while True:
            ...
            for i in range(len(clusters)):
                ...
                shift = sqrt(
                    (ncx - centroids[i][0]) * (ncx - centroids[i][0])
            ...
            if biggest_shift < self.tolerance:
                break
...
class DensityClustering:
    ...
    def findClustersD(self, ptsIndices, ptsTypes=None, cVolcut=50):
        ...
        while True:
            ...
            if keycache.has_key((i - si, j, k)):
                neighbors.append(indcache[(i - si, j, k)])
            if keycache.has_key((i + si, j, k)):
                neighbors.append(indcache[(i + si, j, k)])
            if keycache.has_key((i, j - sj, k)):
                neighbors.append(indcache[(i, j - sj, k)])
            if keycache.has_key((i, j + sj, k)):
                neighbors.append(indcache[(i, j + sj, k)])
            if keycache.has_key((i, j, k - sk)):
                neighbors.append(indcache[(i, j, k - sk)])
            if keycache.has_key((i, j, k + sk)):
                neighbors.append(indcache[(i, j, k + sk)])

            if keycache.has_key((i - si, j + sj, k)):
                neighbors.append(indcache[(i - si, j + sj, k)])
            if keycache.has_key((i + si, j + sj, k)):
                neighbors.append(indcache[(i + si, j + sj, k)])
            if keycache.has_key((i - si, j - sj, k)):
                neighbors.append(indcache[(i - si, j - sj, k)])
            if keycache.has_key((i + si, j - sj, k)):
                neighbors.append(indcache[(i + si, j - sj, k)])
            if keycache.has_key((i - si, j, k + sk)):
                neighbors.append(indcache[(i - si, j, k + sk)])
            if keycache.has_key((i + si, j, k + sk)):
                neighbors.append(indcache[(i + si, j, k + sk)])
            if keycache.has_key((i - si, j, k - sk)):
                neighbors.append(indcache[(i - si, j, k - sk)])
            if keycache.has_key((i + si, j, k - sk)):
                neighbors.append(indcache[(i + si, j, k - sk)])
            if keycache.has_key((i, j - sj, k + sk)):
                neighbors.append(indcache[(i, j - sj, k + sk)])
            if keycache.has_key((i, j + sj, k + sk)):
                neighbors.append(indcache[(i, j + sj, k + sk)])
            if keycache.has_key((i, j - sj, k - sk)):
                neighbors.append(indcache[(i, j - sj, k - sk)])
            ...
            if keycache.has_key((i + si, j + sj, k + sk)):
                neighbors.append(indcache[(i + si, j + sj, k + sk)])
            if keycache.has_key((i - si, j + sj, k + sk)):
                neighbors.append(indcache[(i - si, j + sj, k + sk)])
            if keycache.has_key((i + si, j - sj, k + sk)):
                neighbors.append(indcache[(i + si, j - sj, k + sk)])
            ...
            if (
                len(neighbors) >= nneighbor
            ):  # if point index (i,j,k) has enough neighbors
            ...
            if ci < len(
                cluster
            ):  # more points in this cluster can be checked for neighbors
            ...
            for c in clv:
                ...
                for index in c:
                    ...
                    for ind in range(1, si):
                        if (
                            keycache.has_key((i - ind, j, k))
                            and indcache[(i - ind, j, k)] not in c
                            and indcache[(i - ind, j, k)] not in c2
                        ):
                            c2.append(indcache[(i - ind, j, k)])
                        if (
                            keycache.has_key((i + ind, j, k))
                            and indcache[(i + ind, j, k)] not in c
                            and indcache[(i + ind, j, k)] not in c2
                    ...
                    for ind in range(1, sj):
                        if (
                            keycache.has_key((i, j - ind, k))
                            and indcache[(i, j - ind, k)] not in c
                            and indcache[(i, j - ind, k)] not in c2
                        ):
                            c2.append(indcache[(i, j - ind, k)])
                        if (
                            keycache.has_key((i, j + ind, k))
                            and indcache[(i, j + ind, k)] not in c
                            and indcache[(i, j + ind, k)] not in c2
                    ...
                    for ind in range(1, sk):
                        if (
                            keycache.has_key((i, j, k - ind))
                            and indcache[(i, j, k - ind)] not in c
                            and indcache[(i, j, k - ind)] not in c2
                        ):
                            c2.append(indcache[(i, j, k - ind)])
                        if (
                            keycache.has_key((i, j, k + ind))
                            and indcache[(i, j, k + ind)] not in c
                            and indcache[(i, j, k + ind)] not in c2
                    ...
                    for indi in range(1, si):
                        for indj in range(1, sj):
                            if (
                                keycache.has_key((i - indi, j + indj, k))
                                and indcache[(i - indi, j + indj, k)] not in c
                                and indcache[(i - indi, j + indj, k)] not in c2
                            ):
                                c2.append(indcache[(i - indi, j + indj, k)])
                            if (
                                keycache.has_key((i + indi, j + indj, k))
                                and indcache[(i + indi, j + indj, k)] not in c
                                and indcache[(i + indi, j + indj, k)] not in c2
                    ...
                    for indi in range(1, si):
                        for indk in range(1, sk):
                            if (
                                keycache.has_key((i - indi, j, k + indk))
                                and indcache[(i - indi, j, k + indk)] not in c
                                and indcache[(i - indi, j, k + indk)] not in c2
                            ):
                                c2.append(indcache[(i - indi, j, k + indk)])
                            if (
                                keycache.has_key((i + indi, j, k + indk))
                                and indcache[(i + indi, j, k + indk)] not in c
                                and indcache[(i + indi, j, k + indk)] not in c2
                    ...
                    for indj in range(1, sj):
                        for indk in range(1, sk):
                            if (
                                keycache.has_key((i, j - indj, k + indk))
                                and indcache[(i, j - indj, k + indk)] not in c
                                and indcache[(i, j - indj, k + indk)] not in c2
                            ):
                                c2.append(indcache[(i, j - indj, k + indk)])
                            if (
                                keycache.has_key((i, j + indj, k + indk))
                                and indcache[(i, j + indj, k + indk)] not in c
                                and indcache[(i, j + indj, k + indk)] not in c2
                    ...
                    for indi in range(1, si):
                        for indj in range(1, sj):
                            for indk in range(1, sk):
                                if (
                                    keycache.has_key((i + indi, j + indj, k + indk))
                                    and indcache[(i + indi, j + indj, k + indk)]
                                    not in c
                                    and indcache[(i + indi, j + indj, k + indk)]
                                    not in c2
                                ):
                                    c2.append(indcache[(i + indi, j + indj, k + indk)])
                                if (
                                    keycache.has_key((i - indi, j + indj, k + indk))
                                    and indcache[(i - indi, j + indj, k + indk)]
                                    not in c
