...
class GlfLabels(IndexedGeom):
    ...
    def __init__(self, name=None, check=1, **kw):
        ...
        glf.glfSetSymbolDepth(0.8)  # .2 is C code default value (it is not deep enough)
        ...
        mat.getFrom[mat.ambient] = [mat.diffuse, 1.0]
    ...
    def Draw(self):
        ...
        if self.useGlfDrawLabels:  # use C function from pyglf to draw labels
            ...
            if labTrans is None:
                labTrans = [[0.0, 0.0, 0.0]] * len(centers)
            glf.glfDrawLabels(
                centers,
                labels,
                indices,
                self.billboardRotation,
                self.fontTranslation,
                self.fontRotateAngles,
                self.fontTypeIsVector,
                self.fontScales,
                fontStyles[self.fontStyle],
                fpProp,
                frontAndBack,
                labTrans,
            )
                ...
                if fp:
                    for m in (0, 1, 2, 3, 4):
                        if fp.binding[m] == viewerConst.PER_VERTEX:
                            ...
                            glMaterialWithCheck(face, viewerConst.propConst[m], p[i])
                    if fp.binding[1] != viewerConst.OVERALL:
