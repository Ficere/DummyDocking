...
class Box(IndexedPolygons):
    ...
    def __init__(self, name=None, check=1, **kw):
        ...
        if vertices is not None:
            ...
            self.xside, self.yside, self.zside = numpy.subtract(pt2, pt1)
        else:
            # set up center
    ...
    def getVertsFromCenter(self, center):
        self.center = center
        halfSide = numpy.multiply(
            numpy.array((self.xside, self.yside, self.zside)), 0.5
        )
    ...
    def getVertsFromCornerPts(self, cornerPoints):
        ...
        if self.maxCube or self.minCube:
            ...
            self.yside = side
            self.zside = side
            pt1 = tuple(numpy.add(center, (side / 2.0, side / 2, side / 2)))
...
class GridBox(Box):
    ...
    def Set(self, check=1, redo=1, updateOwnGui=True, **kw):
        ...
        apply(
            self.updateVal,
            (["xnpts", "ynpts", "znpts", "xspacing", "yspacing", "zspacing"],),
            kw,
        )
...
class NiceBox:
    ...
    def __init__(self, name):
        ...
        self.faces = IndexedPolygons(
            ...
            visible=0,
            inheritMaterial=False,
            materials=self.faceColors,
            inheritFrontPolyMode=False,
            frontPolyMode="outlined",
            shading="flat",
            opacity=0.3,
            transparent=True,
        )
    ...
    def updateVertices(self):
        c1 = self.center - self.sides * 0.5
        c2 = self.center + self.sides * 0.5
        self.vertices[:] = [
            (c1[0], c1[1], c2[2]),
            (c2[0], c1[1], c2[2]),
            (c2[0], c2[1], c2[2]),
