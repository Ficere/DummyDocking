...
def RedWhiteRamp(size=256, red=1):
    ramp = numpy.ones( (size, 3), 'f')*red
    incr = 1./(size-1)
    for i in xrange(size):
        ramp[i][1] = i*incr
        ramp[i][2] = i*incr

def GreenWhiteRamp(size=256, green=1):
    ramp = numpy.ones( (size, 3), 'f')*green
    incr = 1./(size-1)
    for i in xrange(size):
        ramp[i][0] = i*incr
        ramp[i][2] = i*incr

def BlueWhiteRamp(size=256, blue=1):
    ramp = numpy.ones( (size, 3), 'f')*blue
    incr = 1./(size-1)
    for i in xrange(size):
        ramp[i][0] = i*incr
        ramp[i][2] = i*incr

def GreenWhiteRed(size=256, upper=1.):
    ramp = numpy.ones( (size, 3), 'f')*upper
    mid = size/2
    incr = 1./(mid-1)
    for i in xrange(mid):
        ramp[i][0] = i*incr
