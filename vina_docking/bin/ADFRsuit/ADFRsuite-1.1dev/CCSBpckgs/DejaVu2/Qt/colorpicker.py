...
class HueSatRadialPicker(QtGui.QWidget):
    ...
    def __init__(self, parent=None, size=150):
        ...
        self.px = None
        for i in range(360):
            self.conical.setColorAt(
                i * 1.0 / 360, QtGui.QColor.fromHsv(i, 255, self.m_value)
            )
    ...
    def buildPixmap(self):
        ...
        rg = QtGui.QRadialGradient(
            self.rect().center(), self.width() / 2 - 2, self.rect().center()
        )
    ...
    def mousePressEvent(self, event):
        if event.button() == QtCore.Qt.LeftButton:
            ...
            QtGui.QWidget.mousePressEvent(event)
    ...
    def setValue(self, val):
        if val < 0 or val > 255 or val == self.m_value:
            return
        self.m_value = val
