...
##             Tkinter.Widget.__init__(self, self.frame, 'togl', cnf, kw)
    ...
    def handleZoomCamera(self, x, y, dx, dy, e ):
        #print 'FUGU', self._orginalx, self._orginaly, x, y, dx, dy, e
...
#        glDisable(GL_CULL_FACE)
    ...
    def pick(self, x, y, dx, dy, e):
        if self.viewer.pickLevel=='both':
            pick = self.DoPick(x, y, type='vertices')
            self.viewer.processPicking(pick)
            pick = self.DoPick(x, y, type='parts')
            self.viewer.processPicking(pick)
        else:
