...
from opengltk.extent.utillib import namedPointsWithNames, namedFaces
...
#        self.Redraw()
    ...
    ##                  glDisableClientState(GL_VERTEX_ARRAY)
                ...
                elif isinstance(obj, IndexedGeom) and type=='parts':
                    v = obj.vertexSet.vertices.array
                    faces = obj.faceSet.faces.array
                    if len(faces):
                        namedFaces(len(faces[0]),
                                   len(v), v.astype('float32'),
                                   len(faces), faces.astype('int32'))
