...
class MoveGeom:
    ...
    def interCameraAttributes(self, ca1, ca2, fraction):
        ...
        cameraAttr["fieldOfView"] = interpolateScalar(
            ca1["fieldOfView"], ca2["fieldOfView"], fraction
        )
        cameraAttr["fogEnd"] = interpolateScalar(ca1["fogEnd"], ca2["fogEnd"], fraction)
        cameraAttr["fogStart"] = interpolateScalar(
            ca1["fogStart"], ca2["fogStart"], fraction
        )
        cameraAttr["lookFrom"] = interpolateVector(
            ca1["lookFrom"], ca2["lookFrom"], fraction
        )
    ...
    def interpolate(
        self, interpolationTime, obj=None, transf=None, camattr=None, interAllobj=False
    ):
        """
        ...
        def _redraw(transf1, transf2, ca1, ca2, fraction):
            if transf1 is not None:
                for _obj in allobjects:
                    qt, tran, scale, pivot = self.interTransformation(
                        _obj, transf1, transf2, fraction
                    )
