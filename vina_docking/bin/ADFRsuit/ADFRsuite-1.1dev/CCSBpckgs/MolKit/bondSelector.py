...
class BondBranchBondSelector(BondSelector):
    ...
    def __init__(self, number):
        self.number = number
        criteria = [
            lambda x, num=number: len(x.atom1.bonds) == num,
...
class BondElementBondSelector(BondSelector):
    ...
    def __init__(self, elem):
        self.elem = elem
        criteria = [
            lambda x, elem=self.elem: x.atom1.element == elem,
...
class PeptideBackBoneBondSelector(BondSelector):
    ...
    def select(self, bnds):
        ...
        bbBnds = BondSet()
        bb_ats = ats.get(
            lambda x: x.parent.type in self.std_res_types and x.name in ["N", "C", "CA"]
        )
        if bb_ats:
...
class AromaticCycleBondSelector2(BondSelector):
    ...
    def select(self, bnds, cutoff=7.5):
        cutoffValue = math.cos(cutoff * math.pi / 180.0)
        ...
        for ring in rf.rings:
            ...
            for bnd in blist:
                ...
                z2 = filter(
                    lambda x: x != bnd and x.atom1 == at or x.atom2 == at, blist
                )
                bnd.nextbond = z2[0]
                neighbor = z2[0].atom1
                if neighbor == at:
                ...
                z2 = filter(
                    lambda x, bnd=bnd, at2=bnd.atom2, blist=blist: x != bnd
                    and x.atom1 == at2
                    or x.atom2 == at2,
                    blist,
                )
...
class PeptideAromaticCycleBondSelector(BondSelector):
    ...
    def select(self, bnds):
        ...
        for i in range(len(aromResSet)):
            ...
            res_bnds = bondDict[i + 1] = res.atoms.get(
                lambda x, keys=keys: x.name in keys
            ).bonds[0]
