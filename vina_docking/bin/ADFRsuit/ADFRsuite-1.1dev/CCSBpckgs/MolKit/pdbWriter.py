...
class PdbWriter(MoleculeWriter):
    ...
    def write(
        ...
        if atmRec is True or hetRec is True or multiMol:
            ...
            self.defineCoordsSection(
                atmInNode, sort=sort, sortFunc=sortFunc, atmRec=atmRec, hetRec=hetRec
            )
        ...
        if not multiMol and not ("  " or " " or "") in [
            x.chemElem for x in mol.allAtoms.data
        ]:
            ...
            if constraints == 1:
                ...
                for value in line:
                    if PDBFormatConstr[key][i] == None:
                        ...
                        assert len(str(value)) <= PDBFormatConstr[key][i]
                    i = i + 1
        ...
        for a in atoms:
            ...
            for b in a.hbonds:
                ...
                if b.donAt.top != b.accAt.top:
                    continue
                if not b.hAt is None and b.donAt.top != b.hAt.top:
                    continue
    ...
    def defineSecondaryStructureSection(self, mol, origin="File"):
        ...
        # 2- Create the SS record from the information generated by stride
            ...
            if not mol.hasSS in ["From Stride", "From PROSS"]:
                return
            else:
                allstrands = SecondaryStructureSet([])
                allhelices = SecondaryStructureSet([])
    ...
    def defineCoordsSection(
        self, nodes, sort=False, sortFunc=None, atmRec=True, hetRec=True
    ):
        """
        ...
        for c in chains:
            ...
            if atmRec:
                ...
                for atm in atoms:
                    self.recordsToWrite["ATOM"].append(
                        self.defineATOM_HETATMRecord(atm)
                    )
                if len(atoms):
    ...
    def defineConnectSection(self, atms, bondOrigin=("File", "UserDefined")):
        ...
        elif type(bondOrigin) is types.StringType and bondOrigin in [
            "File",
            "BuiltByDistance",
            "UserDefined",
        ]:
...
class PdbqWriter(PdbWriter):
    ...
    def write_records(self, file, molecule):
        ...
        for key in tags:
            if self.userRecords.has_key(key) and len(self.userRecords[key]) != 0:
                RecordList = self.userRecords[key]
            elif (
                isPdbParser
                and len(Parser.getRecords(Parser.allLines, "%s" % key)) != 0
                and (
                    (key in mandatory)
                    or (key in required)
                    or self.userRecords.has_key(key)
                )
            ):
