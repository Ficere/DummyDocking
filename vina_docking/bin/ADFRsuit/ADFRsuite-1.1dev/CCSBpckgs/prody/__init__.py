...
def deprecate(dep, alt, ver=None, sl=3):
    ...
    warnings.warn(
        "`{0:s}` is deprecated for removal in v{1:s}, use `{2:s}`.".format(
            dep, ver, alt
        ),
        DeprecationWarning,
        stacklevel=sl,
...
try:
    ...
    LOGGER.warning(
        "prody.__init__.py: Failed to get mgltools resource folder.\nUsing prody's default .prodyrc file"
    )
...
CONFIGURATION = {
    ...
    "verbosity": ("error", list(utilities.LOGGING_LEVELS), LOGGER._setverbosity),
    ...
    if args:
        ...
        for option in args:
            try:
                ...
                raise KeyError(
                    "{0:s} is not a valid configuration option".format(repr(option))
                )
        if len(values) == 1:
            return values[0]
        else:
    ...
    for option, value in kwargs.items():
        try:
            ...
            try:
                ...
                raise TypeError(
                    "{0:s} must be a {1:s}".format(option, type(default).__name__)
                )
            if acceptable is not None and value not in acceptable:
                raise ValueError(
                    "{0:s} must be one of {1:s}".format(
                        option, joinRepr(acceptable, sort=True, last=", or ")
                    )
                )
            ...
            if not silent:
                LOGGER.info(
                    "ProDy is configured: {0:s}={1:s}".format(option, repr(value))
                )
            SETTINGS.save()
...
confProDy.__doc__ += (
    "\n\n"
    + tabulate(["Option"] + _keys, ["Default (acceptable values)"] + _vals)
    + """
...
def checkUpdates():
    ...
    if releases[0] == __version__:
        LOGGER.info(
            "You are using the latest ProDy release (v{0:s}).".format(__version__)
        )
    else:
        LOGGER.info(
            "ProDy v{0:s} is available, you are using {1:s}.".format(
...
if SETTINGS["check_updates"]:
    ...
    if (time.time() - SETTINGS.get("last_check")) / 3600 / 24 > SETTINGS[
        "check_updates"
    ]:
