...
try:
    ...
    try:
        ...
        raise ImportError("You need argparse module to " "be able to use ProDy apps.")
import textwrap

from prody.utilities import wrapText

__all__ = ["Quiet", "UsageExample", "ProDyCitation", "ProDyVersion", "DevelApp"]
...
class UsageExample(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        lines = [line.strip() for line in namespace.usage_example.splitlines()]
        buff = ""
        for line in lines:
...
class DevelApp(object):
    ...
    def _getKwargs(self, arg):
        ...
        if "choices" in kwargs:
            help = ", one of " + ", ".join([str(ch) for ch in kwargs["choices"]])
    ...
    def _docArg(self, doc, arg):
        ...
        if choices:
            desc += ", one of " + ", ".join(["``" + repr(ch) + "``" for ch in choices])
        default = kwargs.get("default")
        if default:
    ...
    def addArgument(self, *args, **kwargs):
        if args in self._args:
            raise ValueError("argument {0} is already defined".format(str(args)))
    ...
    def addApplication(self, parser):
        ...
        if self._example:
            ...
            sub.set_defaults(usage_example=self._example, test_examples=self._egtests)
        ...
        positional = [
            (arg[0], self._args[arg].get("nargs", None))
            for arg in self._group_args["positional"]
        ]
