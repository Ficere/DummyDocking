...
APP = DevelApp("occupancy", help="calculate occupancy of rows and columns in MSA")
...
APP.addArgument("msa", help="MSA file")
...
APP.addArgument(
    ...
    "--prefix",
    dest="prefix",
    help="output filename prefix, default is " "msa filename with _occupancy suffix",
    type=str,
    metavar="STR",
    group="output",
)
    ...
    for i, occ in enumerate(occupancy):
        if kwargs.get("figocc"):
            try:
                ...
                show = showMSAOccupancy(
                    msa=msa, occ=occ, label=label, xlabel=xlabel, title=title
                )
                format = kwargs.get("figformat", "pdf")
                figure.savefig(
                    prefix + suffix[i] + "." + format,
                    format=format,
