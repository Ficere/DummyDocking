...
APP.addArgument("msa", nargs="+", help="MSA filenames to be merged")
...
APP.addArgument(
    ...
    "--outname",
    dest="outname",
    help="output filename, default is first input filename " "with _merged suffix",
    type=str,
    metavar="STR",
    group="output",
)
...
def evol_merge(*msa, **kwargs):
    ...
    outname = kwargs.get("outname") or (
        msafile.getTitle() + "_merged" + MSAEXTMAP[msafile.format]
    )
