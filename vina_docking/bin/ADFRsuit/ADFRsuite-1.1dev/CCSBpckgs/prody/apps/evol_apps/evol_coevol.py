...
APP = DevelApp("coevol", help="analyze co-evolution using mutual information")
...
APP.addArgument("msa", help="refined MSA file")
...
APP.addArgument(
    ...
    "--prefix",
    dest="prefix",
    help="output filename prefix, default is " "msa filename with _coevol suffix",
    type=str,
    metavar="STR",
    group="output",
)
...
def evol_coevol(msa, **kwargs):
    ...
    for what, which in todo:
        ...
        writeArray(
            prefix + suffix + ".txt", matrix, format=kwargs.get("numformat", "%12g")
        )

        if heatmap:
            writeHeatmap(
                prefix + suffix + ".hm", matrix, title=msa.getTitle() + tuffix, **hmargs
            )
        ...
        if kwargs.get("figcoevol"):
            try:
                ...
                show = showMutinfoMatrix(
                    matrix, msa=msa, clim=(cmin, cmax), xlabel=xlabel, title=title
                )
