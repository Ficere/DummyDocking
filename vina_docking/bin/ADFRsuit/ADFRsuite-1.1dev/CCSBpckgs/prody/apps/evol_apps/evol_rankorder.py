...
APP.addArgument("mutinfo", help="mutual information matrix")
...
def evol_rankorder(mutinfo, **kwargs):
    ...
    if not pdbflag:
        if msa is not None:
            ...
            if msa.numResidues() != shape[0]:
                LOGGER.info(
                    "Input MSA and mutinfo do not have similar no "
                ...
                if index is None:
                    if label is not None:
                        LOGGER.info(
                            "Could not find given label in MSA, "
                ...
                if (start and end is not None) and (start < end):
                    ...
                    if len(resnum) != shape[0]:
                        ...
                        LOGGER.info(
                            "Residue numbers will be based on label: "
                            "{0}".format(label)
                        )
                else:
                    LOGGER.info(
                        "Could not identify residue indexes from MSA"
            ...
            LOGGER.info(
                "MSA or PDB not given or does not match mutinfo, "
                "using serial indexing"
            )
            resnum = np.arange(1, shape[0] + 1)

    LOGGER.info(
        "Residue numbers start and end with {0}-{1}".format(
    ...
    if zscore:
        LOGGER.info(
            "zscore normalization applied such that each column "
    ...
    if not kwargs.get("usedist") or not pdbflag:
        if kwargs.get("usedist"):
            LOGGER.info(
                "use-struct-sep set to true, but PDB not given or "
                "incorrect residue number. Using sequence separation"
            )
        else:
            if pdbflag:
                LOGGER.info(
                    "use-dist not set, using sequence separation"
