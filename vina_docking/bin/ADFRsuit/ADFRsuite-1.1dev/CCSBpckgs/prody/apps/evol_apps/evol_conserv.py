...
APP = DevelApp("conserv", help="analyze conservation using Shannon entropy")
...
APP.addArgument(
    ...
    "--prefix",
    dest="prefix",
    help="output filename prefix, default is " "msa filename with _conserv suffix",
    type=str,
    metavar="STR",
    group="output",
)
...
def evol_conserv(msa, **kwargs):
    ...
    writeArray(prefix + ".txt", entropy, format=kwargs.get("numformat", "%12g"))

    if kwargs.get("figent"):
        try:
            import matplotlib.pyplot as plt
        except ImportError:
            LOGGER.warn("Matplotlib could not be imported, " "figures are not saved.")
        else:
            ...
            figure.savefig(
                prefix + "." + format, format=format, dpi=kwargs.get("figdpi", 300)
            )
