...
def prody_contacts(**kwargs):
    ...
    for pdb in ligands:
        ...
        if sel:
            LOGGER.info(
                "{0} atoms from {1} contact {2}.".format(len(sel), pdb, str(target))
            )
            if extend:
                sel = target.select("same " + extend + " as sel", sel=sel)
                LOGGER.info(
                    "Selection is extended to {0} atoms of the same "
...
def addCommand(commands):
    subparser = commands.add_parser(
        "contacts", help="identify contacts between a target and ligand(s)"
    )
    ...
    subparser.set_defaults(
        usage_example="""Identify contacts of a target structure with one or more ligands.
...
contacting residues of the target protein:
    ...
    subparser.add_argument(
        ...
        type=str,
        metavar="SELSTR",
        help="selection string for target",
    )
    ...
    subparser.add_argument("target", help="target PDB identifier or filename")

    subparser.add_argument(
        "ligand", nargs="+", help="ligand PDB identifier(s) or filename(s)"
    )
