...
for key, txt, val in [
    ...
    ("cutoff", "cutoff distance (A)", 15.0),
    ("gamma", "spring constant", 1.0),
    ...
    (
        "figmode",
        "save mode shape figures for specified modes, "
...
def prody_anm(pdb, **kwargs):
    ...
    if select is None:
        LOGGER.warn("Selection {0} did not match any atoms.".format(repr(selstr)))
        return
    LOGGER.info("{0} atoms will be used for ANM calculations.".format(len(select)))
    ...
    if outall or kwargs.get("outeig"):
        prody.writeArray(
            join(outdir, prefix + "_evectors" + ext),
            anm.getArray(),
            delimiter=delim,
            format=format,
        )
        prody.writeArray(
            join(outdir, prefix + "_evalues" + ext),
            anm.getEigvals(),
            delimiter=delim,
            format=format,
        )
    ...
    if outall or kwargs.get("outcov"):
        prody.writeArray(
            join(outdir, prefix + "_covariance" + ext),
            anm.getCovariance(),
            delimiter=delim,
            format=format,
        )
    ...
    if outall or kwargs.get("outcc") or kwargs.get("outhm"):
        ...
        if outall or kwargs.get("outcc"):
            prody.writeArray(
                join(outdir, prefix + "_cross-correlations" + ext),
                cc,
                delimiter=delim,
                format=format,
            )
    ...
    if outall or kwargs.get("hessian"):
        prody.writeArray(
            join(outdir, prefix + "_hessian" + ext),
            anm.getHessian(),
            delimiter=delim,
            format=format,
        )
    ...
    if outall or kwargs.get("kirchhoff"):
        prody.writeArray(
            join(outdir, prefix + "_kirchhoff" + ext),
            anm.getKirchhoff(),
            delimiter=delim,
            format=format,
        )
    ...
    if outall or kwargs.get("outsf"):
        prody.writeArray(
            join(outdir, prefix + "_sqflucts" + ext),
    ...
    if figall or cc or sf or bf or cm:
        try:
            ...
            LOGGER.warning(
                "Matplotlib could not be imported. " "Figures are not saved."
            )
        else:
            ...
            if figall or cc:
                ...
                plt.savefig(
                    join(outdir, prefix + "_cc." + format), dpi=dpi, format=format
                )
                plt.close("all")
            ...
            if figall or cm:
                ...
                plt.savefig(
                    join(outdir, prefix + "_cm." + format), dpi=dpi, format=format
                )
                plt.close("all")
            ...
            if figall or sf:
                ...
                plt.savefig(
                    join(outdir, prefix + "_sf." + format), dpi=dpi, format=format
                )
                plt.close("all")
            ...
            if figall or bf:
                ...
                plt.plot(
                    bcal,
                    label=(
                        "Theoretical (R={0:.2f})".format(np.corrcoef(bcal, bexp)[0, 1])
                    ),
                )
                ...
                plt.savefig(
                    join(outdir, prefix + "_bf." + format), dpi=dpi, format=format
                )
                plt.close("all")
...
for key in _:
    prody_anm.__doc__ += """
    :arg {0}: {1}, default is ``{2!r}``""".format(
        key, HELPTEXT[key], DEFAULTS[key]
    )


def addCommand(commands):
    subparser = commands.add_parser(
        "anm", help="perform anisotropic network model calculations"
    )

    subparser.add_argument(
        "--quiet", help="suppress info messages to stderr", action=Quiet, nargs=0
    )

    subparser.add_argument(
        "--examples", action=UsageExample, nargs=0, help="show usage examples and exit"
    )
    subparser.set_defaults(
        usage_example="""Perform ANM calculations for given PDB structure and output results in NMD
...
graphical output files:
  ...
  $ prody anm 1aar -s "calpha and chain A and resnum < 70" -A""",
    ...
    group.add_argument(
        ...
        dest="hessian",
        action="store_true",
        default=DEFAULTS["hessian"],
        help=HELPTEXT["hessian"],
    )
    ...
    group.add_argument(
        ...
        dest="kirchhoff",
        action="store_true",
        default=DEFAULTS["kirchhoff"],
        help=HELPTEXT["kirchhoff"],
    )
    ...
    group.add_argument(
        ...
        dest="figbeta",
        action="store_true",
        default=DEFAULTS["figbeta"],
        help=HELPTEXT["figbeta"],
    )
    ...
    group.add_argument(
        ...
        dest="figcmap",
        action="store_true",
        default=DEFAULTS["figcmap"],
        help=HELPTEXT["figcmap"],
    )
    ...
    subparser.set_defaults(
        func=lambda ns: prody_anm(ns.__dict__.pop("pdb"), **ns.__dict__)
    )
