...
def addCommand(commands):
    subparser = commands.add_parser("fetch", help="fetch a PDB file")
    ...
    subparser.set_defaults(
        usage_example="""Download PDB file(s) by specifying identifiers:
  ...
  $ prody fetch 1mkp 1p38""",
    ...
    subparser.add_argument(
        ...
        action="store_true",
        default=False,
        help="write compressed PDB file(S)",
    )

    subparser.add_argument(
        "pdb", nargs="+", help="PDB identifier(s) or a file that contains them"
    )
