...
def prody_catdcd(*dcd, **kwargs):
    ...
    if ag is None and (align or select):
        raise ValueError(
            "one of PSF or PDB files must be provided for "
    ...
    if ag:
        ...
        if select:
            ...
            if select is None:
                raise ValueError(
                    "{0} did not match any atoms".format(repr(kwargs.get("select")))
                )
        else:
            select = ag
        LOGGER.info("{0} atoms are selected for writing output.".format(len(select)))
        if align:
            align = ag.select(align)
            traj.setAtoms(align)
            LOGGER.info(
                "{0} atoms are selected for aligning frames.".format(len(align))
            )
    ...
    goto = stride != 1
    slc = slice(kwargs.get("first", 0), kwargs.get("last", -1), stride).indices(
        len(traj) + 1
    )
    for i in range(*slc):
        if goto:
    ...
    traj.close()
    out.close()
    LOGGER.info("{0} frames are written into {1}.".format(count, output))


def addCommand(commands):
    subparser = commands.add_parser("catdcd", help="concatenate dcd files")
    ...
    subparser.set_defaults(
        usage_example="""Concatenate two DCD files and output all atmos:
...
Concatenate two DCD files and output backbone atoms:
  ...
  $ prody catdcd mdm2.dcd mdm2sim2.dcd --pdb mdm2.pdb -s bb"""
    ...
    subparser.add_argument(
        "--psf", help="PSF filename (must have same number of atoms as DCDs)"
    )
    subparser.add_argument(
        "--pdb", help="PDB filename (must have same number of atoms as DCDs)"
    )
    ...
    subparser.add_argument(
        "dcd", nargs="+", help="DCD filename(s) (all must have same number of atoms)"
    )

    subparser.set_defaults(subparser=subparser)
    subparser.set_defaults(
        func=lambda ns: prody_catdcd(*ns.__dict__.pop("dcd"), **ns.__dict__)
    )
