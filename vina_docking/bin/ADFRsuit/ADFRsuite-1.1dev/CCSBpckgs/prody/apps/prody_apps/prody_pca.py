...
for key, txt, val in [
    ...
    (
        "figproj",
        "save projections onto specified subspaces, e.g. "
...
def prody_pca(coords, **kwargs):
    ...
    if ext == ".dcd":
        ...
        if pdb:
            if pdb.numAtoms() == dcd.numAtoms():
                ...
                dcd.setAtoms(select)
                LOGGER.info(
                    "{0} atoms are selected for calculations.".format(len(select))
                )
            else:
                select = pdb.select(selstr)
                if select.numAtoms() != dcd.numAtoms():
                    raise ValueError(
                        "number of selected atoms ({0}) does "
                        "not match number of atoms in the DCD "
                        "file ({1})".format(select.numAtoms(), dcd.numAtoms())
                    )
                if pdb.numCoordsets():
        ...
        select = pdb.select(selstr)
        LOGGER.info("{0} atoms are selected for calculations.".format(len(select)))
        if select is None:
            raise ValueError(
                "selection {0} do not match any atoms".format(repr(selstr))
            )
        LOGGER.info("{0} atoms will be used for PCA calculations.".format(len(select)))
    ...
    if extend:
        if pdb:
            ...
            prody.writeNMD(
                join(outdir, prefix + "_extended_" + extend + ".nmd"), *extended
            )
        else:
            prody.LOGGER.warn(
                "Model could not be extended, provide a PDB or " "PSF file."
            )
    ...
    if outall or kwargs.get("outsf"):
        prody.writeArray(
            join(outdir, prefix + "_sqfluct" + ext),
        ...
        try:
            ...
            if figall or sp:
                ...
                for index in indices:
                    ...
                    plt.savefig(
                        join(
                            outdir, prefix + "_proj_" + "_".join(index) + "." + format
                        ),
                        dpi=dpi,
                        format=format,
                    )
...
def addCommand(commands):
    subparser = commands.add_parser(
        "pca", help="perform principal component analysis calculations"
    )
    ...
    subparser.set_defaults(
        usage_example="""This command performs PCA (or EDA) calculations for given multi-model \
...
Perform EDA for backbone atoms:
  ...
  $ prody eda mdm2.dcd --pdb mdm2.pdb --select backbone""",
    ...
    group.add_argument(
        ...
        dest="outproj",
        action="store_true",
        default=DEFAULTS["outproj"],
        help=HELPTEXT["outproj"],
    )
    ...
    subparser.add_argument(
        ...
        dest="aligned",
        action="store_true",
        default=DEFAULTS["aligned"],
        help=HELPTEXT["aligned"],
    )
