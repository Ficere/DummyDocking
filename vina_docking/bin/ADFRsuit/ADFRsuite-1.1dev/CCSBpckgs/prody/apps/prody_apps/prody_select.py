...
def prody_select(selstr, *pdbs, **kwargs):
    ...
    if (
        (isfile(selstr) or len(selstr) == 4 and selstr[0].isdigit())
        and len(pdbs) == 1
        and not isfile(pdbs[0])
    ):
        pdbs, selstr = selstr, pdbs[0]
        LOGGER.warn(
            "The order of selstr and pdb arguments have switched "
    ...
    for pdb in pdbs:
        ...
        if pdbselect is None:
            ...
            return
        LOGGER.info(
            "Selection {0} matched {1} atoms.".format(repr(selstr), len(pdbselect))
        )

        outname = output or ((prefix or pdb.getTitle()) + suffix)
        LOGGER.info("Selection is written into: " + writePDB(outname, pdbselect))


def addCommand(commands):
    subparser = commands.add_parser("select", help="select atoms and write a PDB file")
    ...
    subparser.set_defaults(
        usage_example="""This command selects specified atoms and writes them in a PDB file.
...
Fetch PDB files 1p38 and 1r39 and write backbone atoms in a file:
  ...
  $ prody select backbone 1p38 1r39""",
    ...
    group.add_argument(
        ...
        metavar="STR",
        type=str,
        help="output PDB filename (default: pdb_selected.pdb)",
    )
    ...
    subparser.set_defaults(
        func=lambda ns: prody_select(ns.select, *ns.pdb, **ns.__dict__)
    )
