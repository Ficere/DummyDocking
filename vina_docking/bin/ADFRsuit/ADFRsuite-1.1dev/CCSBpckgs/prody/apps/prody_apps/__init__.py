...
prody_parser = argparse.ArgumentParser(
    ...
    epilog="See 'prody <command> -h' for more information on a specific " "command.",
)

prody_parser.add_argument(
...
prody_commands = prody_parser.add_subparsers(title="subcommands")


for cmd in PRODY_APPS:
    cmd = "prody_" + cmd
    mod = imp.load_module(
        "prody.apps.prody_apps." + cmd, *imp.find_module(cmd, [path_apps])
    )
