...
def prody_align(*pdbs, **kwargs):
    ...
    if len(pdbs) == 1:
        ...
        LOGGER.info("{0} atoms will be used for alignment.".format(len(pdbselect)))
        ...
        if ref_sel:
            LOGGER.info(
                "Selection {0} matched {1} atoms.".format(repr(selstr), len(ref_sel))
            )
        else:
            raise ValueError(
                "selection {0} did not match any atoms".format(repr(selstr))
            )
        match = True
        ...
        for arg in pdbs:
            ...
            LOGGER.info(
                "Selection {0} matched {1} atoms.".format(repr(selstr), len(pdb_sel))
            )
            if len(pdb_sel) == len(ref_sel) and all(
                pdb_sel.getNames() == ref_sel.getNames()
            ):
...
def addCommand(commands):
    subparser = commands.add_parser("align", help="align models or structures")
    ...
    subparser.set_defaults(
        usage_example="""Align models in a PDB structure or multiple PDB structures and save \
...
Align 1fi7 onto 1hrc using heme atoms:
    ...
    subparser.add_argument("pdb", nargs="+", help="PDB identifier(s) or filename(s)")

    subparser.set_defaults(func=lambda ns: prody_align(*ns.pdb, **ns.__dict__))
