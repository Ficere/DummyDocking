    ...
    for i, biomol in enumerate(biomols):
        if isinstance(biomol, prody.Atomic):
            ...
            for j, part in enumerate(biomol):
                outfn = "{0}_biomol_{1}_part_{2}.pdb".format(prefix, i + 1, j + 1)
                LOGGER.info("Writing {0}".format(outfn))
                prody.writePDB(outfn, part)


def addCommand(commands):
    subparser = commands.add_parser("biomol", help="build biomolecules")
    ...
    subparser.set_defaults(
        usage_example="""Generate biomolecule coordinates:
