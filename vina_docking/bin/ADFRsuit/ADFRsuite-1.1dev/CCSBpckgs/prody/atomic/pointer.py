...
class AtomPointer(Atomic):
    ...
    def __init__(self, ag, acsi):
        if not isinstance(ag, AtomGroup):
            raise TypeError(
                "ag must be an AtomGroup instance, not {0}".format(type(ag))
            )
        self._ag = ag
        if acsi is None:
    ...
    def __contains__(self, item):
        try:
            ...
            return (
                self._ag == ag
                and len(item) <= len(self)
                and set(item._getIndices()).issubset(set(self._getIndices()))
            )
    ...
    def __eq__(self, other):
        try:
            ...
            return (
                self._ag == ag
                and other.numAtoms() == self.numAtoms()
                and all(self._getIndices() == other._getIndices())
            )
    ...
    def __or__(self, other):
        ...
        if acsi != other.getACSIndex():
            LOGGER.warn(
                "Active coordinate set indices do not match, it will " "be set to zero."
            )
            acsi = 0
        ...
        return Selection(
            self._ag,
            indices,
            "({0}) or ({1})".format(self.getSelstr(), other.getSelstr()),
            acsi,
            unique=True,
        )
    ...
    def __and__(self, other):
        ...
        if acsi != other.getACSIndex():
            LOGGER.warning(
                "active coordinate set indices do not match, "
        ...
        if indices:
            ...
            return Selection(
                self._ag,
                indices,
                "({0}) and ({1})".format(self.getSelstr(), other.getSelstr()),
                acsi,
            )
    ...
    def __add__(self, other):
        ...
        return AtomMap(ag, indices, acsi, title=title, intarrays=True, dummies=dummies)
    ...
    def _getSubset(self, label):
        subset = array(
            list(set(self._ag._getSubset(label)).intersection(set(self._getIndices()))),
            int,
        )
        subset.sort()
        return subset
