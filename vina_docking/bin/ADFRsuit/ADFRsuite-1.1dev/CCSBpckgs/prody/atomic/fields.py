...
class Field(object):
    ...
    def getDocstr(self, meth, plural=True, selex=True):
        ...
        if selex and selstr:
            ...
            selex = ("  {0} can be used in atom selections, e.g. " "``{1}``.").format(
                doc.capitalize(), selex
            )
            if self.synonym is not None:
                selex = selex + ("  Note that *{0}* is a synonym for " "*{1}*.").format(
...
ATOMIC_FIELDS = {
    ...
    "anisou": Field("anisou", float, doc="anisotropic temperature factor", ndim=2),
    "chain": Field(
        "chain",
        DTYPE + "2",
    ...
    "element": Field(
        "element", DTYPE + "2", doc="element symbol", selstr=("element C O N",)
    ),
    "occupancy": Field(
        "occupancy",
        float,
    ...
    "resname": Field(
        "resname", DTYPE + "6", doc="residue name", selstr=("resname ALA GLY",)
    ),
    "resnum": Field(
        "resnum",
        int,
    ...
    "siguij": Field(
        "siguij",
        float,
        doc="standard deviations for " "anisotropic temperature factor",
        meth="Anistd",
        ndim=2,
    ),
    "serial": Field(
        "serial",
        int,
        doc="serial number (from file)",
        doc_pl="serial numbers (from file)",
        none=["_sn2i"],
        selstr=("serial 1 2 3", "serial 1 to 10", "serial 1:10:2", "serial < 10"),
    ),
    "beta": Field(
        "beta",
        float,
        doc="β-value (temperature factor)",
        doc_pl="β-values (or temperature factors)",
        selstr=("beta 555.55", "beta 0 to 500", "beta 0:500", "beta < 500"),
    ),
    "icode": Field(
        "icode",
        DTYPE + "1",
    ...
    "mass": Field(
        "mass", float, doc_pl="masses", meth_pl="Masses", selstr=("12 <= mass <= 13.5",)
    ),
    "radius": Field(
        "radius",
        float,
...
for key in keys:
    ...
    __doc__ += wrapText(
        field.doc + (" *(read only)*" if field.readonly else ""), indent=6
    )
