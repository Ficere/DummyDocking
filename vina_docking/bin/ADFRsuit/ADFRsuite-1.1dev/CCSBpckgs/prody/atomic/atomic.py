            ...
            if name.startswith("is") and self.isFlagLabel(name[2:]):
                ...
                if name == "all":
                    try:
                        ...
                        selstr = name
                        return Selection(
                            ag, arange(self.numAtoms()), "all", self._acsi, unique=True
                        )
                    else:
                        try:
                    ...
                    try:
                        ...
                        selstr = "({0}) and ({1})".format(name, self.getSelstr())
                    try:
                        dummies = self.numDummies()
                    except AttributeError:
                        ...
                        if len(indices):
                            return Selection(
                                ag, indices, selstr, self._acsi, unique=True
                            )
                        else:
                            return None
                    else:
                        ...
                        if len(indices):
                            return AtomMap(
                                ...
                                intarrays=True,
                                dummies=dummies,
                                title="Selection " + repr(name) + " from " + str(self),
                            )
                        else:
                            return None
                else:
                    ...
                    word = items[0]
                    if (
                        self.isFlagLabel(word)
                        or self.isDataLabel(word)
                        or word in NOTALLNONE
                        or isSelectionMacro(word)
                    ):
                        selstr = " ".join(items)
                        return SELECT.select(self, selstr)

        raise AttributeError(
            "{0} object has no attribute `{1}` and {2} "
            "is not a valid selection string".format(
                self.__class__.__name__, name, repr(selstr)
            )
        )

    def __getstate__(self):
        return dict([(slot, getattr(self, slot)) for slot in self.__class__.__slots__])
        ...
        if bonds is not None and bmap is not None:
            if indices is None:
                ...
                if len(set(indices)) == len(indices):
                    ...
                    LOGGER.warn("Duplicate atoms in mapping, bonds are " "not copied.")
            else:
