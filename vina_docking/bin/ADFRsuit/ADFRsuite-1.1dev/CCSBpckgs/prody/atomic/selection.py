...
class Selection(AtomSubset):
    ...
    * :func:`iter` yields :class:`.Atom` instances"""
    ...
    def __repr__(self):
        ...
        if n_csets:
            if n_csets == 1:
                return ("<{0}: {1} from {2} ({3} atoms)>").format(
                    self.__class__,
                    repr(selstr),
                    self._ag.getTitle(),
                    len(self),
                ...
                return (
                    "<{0}: {1} from {2} ({3} atoms; " "active #{4} of {5} coordsets)>"
                ).format(
                    self.__class__,
                    repr(selstr),
                    self._ag.getTitle(),
            ...
            return ("<{0}: {1} from {2} ({3} atoms; no " "coordinates)>").format(
                self.__class__, repr(selstr), self._ag.getTitle(), len(self)
            )
