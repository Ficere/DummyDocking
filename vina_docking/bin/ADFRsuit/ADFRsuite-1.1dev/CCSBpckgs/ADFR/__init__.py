...
def getADelements():
    """ get list of elements known by AD4"""
    from ADFRcc import getFFParameters
    parameters = getFFParameters()
    ADAtomTypes = {}
    for i in range(parameters.numAtomTypes):
        # two lines to cover Upper case symbols and lower case (like BR and Br)
        ADAtomTypes[parameters.getAtomTypeByIndex(i).element.symbol.upper()] = True
        ADAtomTypes[parameters.getAtomTypeByIndex(i).element.symbol] = True
    del ADAtomTypes['X'] # added in ADFRcc code
    return ADAtomTypes

ADelements = getADelements()

def PDBQTlinesForAtoms(atoms):
    names = atoms.getNames()
    resnames = atoms.getResnames()
    resnums = atoms.getResnums()
    chids = atoms.getChids()
    altoc = atoms.getAltlocs()
    if altoc is None:
        altoc = [' ']*len(atoms)
    ...
    occup = atoms.getOccupancies()
    if occup is None:
        occup = [1.0]*len(atoms)
    tfact = atoms.getBetas()
    if tfact is None:
        tfact = [0.0]*len(atoms)
    charges = atoms.getCharges()
    segnames = atoms.getSegnames()
    if segnames is None:
        segnames = ['']*len(atoms)
    ADelems = atoms.getData('AD_element')
    elems = atoms.getData('element')
    ...
    for ind, atom in enumerate(atoms):
        ...
        if len(name)==4 and name[-1].isdigit():
            name = name[3]+name[:3]
        elif len(name)==4 and (atom.nucleic or atom.hetero):
            # atoms from nucleic acids with 4 letter names are shifted to the left
            # eg 201d.pdb 
#ATOM     30  H22  DG A   1      -5.156   7.845  -3.531  1.00  0.53           H  
#ATOM     31 HO5'  DG A   1      -4.038  11.195  -5.709  1.00  0.90           H  
            name = '%-4s'%name
        elif len(elems[ind])==2:
            elems[ind] = elems[ind][0]+elems[ind][1].lower()
            name = '%-4s'%name
        else:
            name = ' %-3s'%name
        icode = icodes[ind]
        ...
        lines.append(
            PDBQT_FMT%(
                key, indices[ind], name, altoc[ind],
                resnames[ind], chids[ind],
                resnums[ind], icode, coords[ind][0],
                coords[ind][1], coords[ind][2],
                occup[ind], tfact[ind],
                segnames[ind], charges[ind], ADelems[ind]))
...
class PDBQTWriter:
    ...
    def getNodeLines(self, root, atIndices):
        for ind in atIndices:
            ...
            if len(name)==4 and name[-1].isdigit():
                name = name[3]+name[:3]
            elif len(self.elems[serial])==2 or len(name.strip())==4:
                name = '%-4s'%name
            else:
                name = ' %-3s'%name
...
def ligandFTfromFile(filename, mol=None, covalentLigandAnchorAtoms=None,
                     fixedRoot=False, neighborSearchCutoff=-1., useRama=True):
...
def molToAtomSetStatic(mol):
    from ADFRcc.adfr import AtomSetStatic
    ag = mol._ag
...
class ADFR:
    ...
    def __init__(self, ligand, mapsFolder, receptor=None,
                 ...
                 localSearchMode='BFGS', neighborSearchCutoff=-1.,useRama=True):
        ...
        *anchor*.npy in mapsFolder. Else if tpointsFilename starts with a path
        ...
        if FTRecsrc is not None:
        ...
        elif flexibleResidues is not None and len(flexibleResidues):
            ...
            # self.FT.allMotions contains all FTTorsion motion objects (i.e. flattened for C++)
            self.FT.allMotions = self.ligandFT.motions + self.RFTGen.allMotions
            self.FT._ramaTors = self.ligandFT._ramaTors
            self.FT.ftRoot = self.FT
            self.FT.receptorAtomSet = self.RFTGen.atomSet
        else:
        ...
        if not covLigAnchorAt and tpointsFilename is not None and not fixedRoot and neighborSearchCutoff < 0:
    ...
    def createPopulation(self, size, neighborSearchCutoff=-1.):
        ...
        genome = GenomePy(self.FT, self.scorer, scaleRE=scaleRE, useRama=self.useRama)
    ...
    def savePopulationGenes(self, pop, filename):
        # saves the population genes
        # one individual per line
        f = open(filename, 'w')
        for i, ind in enumerate(pop):
            [f.write('%f '%x) for x in ind.getGenesPy()]
            f.write('%f\n'%-ind.score())
        f.close()
    ...
    def createNeighborPopulation(self, size, neighborSearchCutoff):
        ...
        genome = GenomePy(self.FT, self.scorer, scaleRE=scaleRE, neighborSearchCutoff=neighborSearchCutoff, useRama=self.useRama)
        ...
        for motion in self.FT.motions:
            if isinstance(motion, PyFTDiscreteTranslation):
                ...
                fttrans = motion
                delta = 1. / (numpy.array(fttrans.getBoundsMax()) - numpy.array(fttrans.getBoundsMin()))
        ...
        while len(pop) < size:
            ...
            ##         dx = numpy.array(
            ##             ([uniform(-1, 1), uniform(-1, 1), uniform(-1, 1)]) * delta * neighborSearchCutoff / 4)
            ...
            if ind._neighborRMSD < neighborSearchCutoff:
                neighborPop += 1
                pop.append(ind)
                rmsds. append(ind._neighborRMSD)
            else:
                count += 1
                if count > 1000000:
                    raise RuntimeError('unable to generate initial neighborhood population')
            #elif len(pop) < size * 0.9 or neighborPop > size * 0.1:
            #    pop.append(ind)
            #print neighborPop,len(pop), ind._neighborRMSD
            #print ct, len(pop), rmsd, aligner.translationMatrix
        print "built neigborhood population of %d in %.2f seconds <rmsd> %5.2f std %5.2f min %5.2f max %5.2f" % (
            size, time() - t0, numpy.average(rmsds), numpy.std(rmsds), min(rmsds), max(rmsds))
        return pop
    ...
    def createLocalPopulation(self, size, radius=2.0): 
        ...
        genome = GenomePy(self.FT, self.scorer, useRama=self.useRama)
    ...
    def createGA(self, pop, referenceLigFile=None,
                 quickmini=None, hardmini=None, seedValue=-1,
                 RMSDMatching='hungarian',
                 neighborSearchCutoff=-1.0,
                 savebestLig=False,
                 clusterEnergyCut=2.0,
        ...
        self.ga = ga = GA(pop, self.ligand, refLig, folder=self.folder,
                          ...
                          localSearchMode=self.localSearchMode,
                          neighborSearchCutoff=neighborSearchCutoff,
                          savebestLig=savebestLig,
                          clusterEnergyCut=clusterEnergyCut,
                          clusteringRMSDCutoff=clusteringRMSDCutoff)
        if quickmini:
