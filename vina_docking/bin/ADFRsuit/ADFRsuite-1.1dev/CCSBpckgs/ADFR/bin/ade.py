...
parser.add_option("--mapFilesRoot",
                  ...
                  dest="mapFilesRoot",
                  help="list of flexbile receptor side chains e.g. [('GLU', '167'), ('TYR', '628')]",)
parser.add_option('-T', "--transPoints",
...
# divide by 2 because symmetrical scorer for ligand IE
scorer = genome.scorer.getLlPairwiseScorer()
sv = numpy.sum(scorer.getVdwEnergyMatrix())*.5
print "unweighted vdw: ", sv
se = numpy.sum(scorer.getEstatEnergyMatrix())*.5
print "unweighted e: ", se
shb = numpy.sum(scorer.getHbEnergyMatrix())*.5
print "unweighted hb: ", shb
sds = numpy.sum(scorer.getSolvEnergyMatrix())*.5
print "unweighted ds: ", sds
print 'Total weighted:', sv*feCoeffVdw + se*feCoeffEstat + shb*feCoeffHbond + sds*feCoeffDesolv
