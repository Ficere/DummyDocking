...
class GenomePy(Genome):
    ...
    def __init__(self, ft, scorer, scaleRE=None, neighborSearchCutoff=-1.0, useRama=True):
        ...
        if ft._ramaTors and useRama:
            print 'SETTING RAMA', ft._ramaTors
            self.setRamaTorsions(ft._ramaTors)
...
class IndividualPy(Individual):
    ...
    def clone(self):
        ...
        ind._neighborRMSD = self._neighborRMSD
        ind._nbExplore = self._nbExplore
...
class GA:
    ...
    def iniBestLigArray(self, ind):
        self._bestLigInGen = []
        if ind.phenotype[0] is not None:
            self._bestLigInGen = [ind.phenotype[0].copy(), ind.phenotype[1].copy()]
        else: # rigid receptor
            self._bestLigInGen = [None, ind.phenotype[1].copy()]

    def setBestLigCoords(self, ind):
        with self._bsetLigLock:
            if self._bestLigInGen[0]:
                self._bestLigInGen[0][:] = ind.phenotype[0]
            self._bestLigInGen[1][:] = ind.phenotype[1]
        
    def getBestLigCoords(self):
        with self._bsetLigLock:
            if self._bestLigInGen[0]:
                return [self._bestLigInGen[0].copy(), self._bestLigInGen[1].copy()]
            else:
                return [None, self._bestLigInGen[1].copy()]            
        
    def __init__(self, pop, ligand, reflig=None, folder='.',
                 RMSDMatching='hungarian', localSearchMode='BFGS',
                 neighborSearchCutoff=-1, savebestLig=False,
                 clusterEnergyCut=2.0, clusteringRMSDCutoff=2.0):

        self.debug = False
        self.status = 'stopped'
        self._endingStatus = None # will hold termination criterion after evolve
        self.gen = 0
        self.pop = pop

        self.iniBestLigArray(pop[0])
        self.savebestLig = savebestLig
        self._bsetLigLock = Lock()
        self.localSearchMode = localSearchMode
        if localSearchMode=='BFGS':
            from ADFRcc.adfr import BFGSQuat
            # using the singleton works if runOneGA is restarted as
            # a process, but if the same process emulatres runOneGA multiple
            # times (e.g. testDocking from gridGUI) the BGFS minimizer
            # won't work after first time
            #self.bfgs = BFGSQuat.getSingleton(pop[0])
            self.bfgs = BFGSQuat(pop[0])
            self.minimize = self.minimizeBFGS
            # clustering Energy cutoff
            self.clusterEnergyCut = clusterEnergyCut
            print 'Local search mode set to BFGS'
            self.quickmini = {
                "aMaxSteps" : 20,
                "xtol_abs" : -1,
                "xtol_rel" : -1,
            ...
            self.minimize = self.minimizeSW
            # clustering Energy cutoff
            self.clusterEnergyCut = clusterEnergyCut
            print 'Local search mode set to SolisWets'
	...
	self.clusteringCutoff = clusteringRMSDCutoff
        self.neighborSearchCutoff = neighborSearchCutoff
    ...
    def explore(self, individual, *args, **kw):#nbSteps=5, noImproveStop=2
        if individual._nbExplore > 2:
            return individual._score, 0
        genome = individual.genomePy
        best = individual.clone()
        startScore = bestScore = best.score()
        current = individual.clone()
        #print 'start', bestScore
        startEvals = genome.scorer.getNumEvals()
        i = noImprov = 0
        while True:
            new = current.clone()
            dev = genome.genDeviatePy(0.3)
            new.setGenes(best.getGenesPy()+dev*10)
            newScore = new.calculateScores()
            sc, nb = self.minimizeBFGS(new, **kw)
            if sc > bestScore:
                bestScore = sc
                best = new.clone()
                nevals = genome.scorer.getNumEvals()
            else:
                acceptance_probability = exp((newScore - current._score) * temperature)
            ...
            if noImprov>=10:
                break
            i += 1
        individual.setGenes(best.getGenesPy())
        score = individual.score()
        #print '        EXPLORE: %.3f -> %.3f %d (%d)'%(
        #    startScore, bestScore, individual._nbExplore, genome.scorer.getNumEvals()-startEvals)
        if score==startScore:
            individual._nbExplore += 1
        else:
            individual._nbExplore = 0
        return score, genome.scorer.getNumEvals()-startEvals
    
    def minimizeBFGS(self, individual, *args, **kw):#nbSteps=5, noImproveStop=2, max_steps=20, MAX_FAIL=3, searchRate=0.3):
        ...
        #score = self.bfgs.minimize(individual, 100, -1, -1, 0.1, -1)
        score = self.bfgs.minimize(individual, **kw)

        individual.score() # force the Python Individual to update _score and phenotype etc...
        #individual._score = score
        #if individual.genomePy.ft.receptorAtomSet:
        #    individual.phenotype[0] = individual.genomePy.ft.receptorAtomSet.getCoordsPy()
        #individual.phenotype[1] = individual.genomePy.ft.ligandAtomSet.getCoordsPy()

        return score, self.bfgs.nevals
    
    def minimizeSW(self, individual, *args, **kw):#, nbSteps=5, noImproveStop=2, max_steps=20, MAX_FAIL=3, searchRate=0.3):
        nbSteps = kw.get('nbSteps', 5)
        noImproveStop = kw.get('noImproveStop', 2)
        max_steps = kw.get('max_steps', 20)
        MAX_FAIL = kw.get('MAX_FAIL', 4)
        MAX_SUCCESS = kw.get('MAX_SUCCESS', 3)
        searchRate = kw.get('searchRate', 0.3)
        aMaxEvals = kw.get('aMaxEvals', -1)
        trajFilename = kw.get('trajFilename', None)
        if _intelContest_ and self._nbWritten< len(self.pop):
            [self._f.write("%.3f "%x) for x in individual.getGenesPy()]
            self._f.write("\n")
            self._nbWritten += 1
        ...
        #Solis Wets
        #print 'SW params', nbSteps, noImproveStop, max_steps, MAX_FAIL, searchRate, aMaxEvals
        self.sw.minimize(individual, nbSteps, noImproveStop, max_steps, MAX_FAIL, searchRate, aMaxEvals,
                         trajFilename)
        #self.sw.minimize(individual, 1, 1, 100, MAX_FAIL, searchRate, aMaxEvals)
        score = individual.score()

        #print "MINI %f %f (%d) %f (%d)"%(before, sc, self.bfgs.nevals, score, self.sw.getSteps())
        #if score > sc:
        #    print 'genesStart', genesStart
        #    print 'bf', indc.getGenesPy()
        #    print 'sw', individual.getGenesPy()
        #    import pdb; pdb.set_trace()
        return score, self.sw.getSteps()
    ...
    def evolve(self, maxGens=1000, p_crossover=0.8, p_mutation=0.05,
               maxEvals=1500000, popStdev=1e-4, noImproveStop=5, verbose=3):
        ...
        while (self.gen <= self.MaxGens and self.pop[0].genomePy.scorer.getNumEvals()<=self.maxEvals) \
            ...
            # base best pose coordinates in threadsafe array
            if self.savebestLig:
                self.setBestLigCoords(self.pop[0])
        ...
        if status=='searching':
            if self.gen > self.MaxGens:
                self._endingStatus = 'MaxGen %d reached'%self.MaxGens
            elif self.pop[0].genomePy.scorer.getNumEvals()>self.maxEvals:
                self._endingStatus = 'MaxEvals %d reached'%self.maxEvals
        else:
            self._endingStatus = status
        self.status = 'ended'
        return status
            
    def cluster(self, remainder, RMSDcalc=None):
        clusters = []
        seedInd = remainder[0]
        pop = self.pop
        if RMSDcalc is None:
    ...
    def step(self):
        ...
        if len(remainder)>2 and self.useClustering:
            ...
            ediffCluster = 0.0
            ediffsCluster = [0.]
            #for cnum, c in enumerate(clusters[:min(len(clusters), 15)]):
            for cnum, c in enumerate(clusters):
                ...
                if cnum < min(len(self.clustersBestE), 5):
                #if cnum < len(self.clustersBestE):
                    diff = self.pop[rep]._score - self.clustersBestE[cnum]
                    ediffCluster += diff
                    ediffsCluster.append(diff)
                if self.localSearchMode=='BFGS':
                    _sc, _nevals = self.explore(self.pop[rep], **self.hardmini)
                else:
                     _sc, _nevals = self.minimize(self.pop[rep], **self.hardmini)
            ...
            if len(clusters) and ediffCluster==0:#0.0001:
                self.nbNoClusterEimprovement += 1
                if self.nbNoClusterEimprovement==self.stopNoImprove:
                    return "no improvement in best %d clusters for %d generations"%(
