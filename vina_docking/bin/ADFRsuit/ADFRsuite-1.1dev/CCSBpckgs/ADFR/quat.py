...
def randomQuat():
    t1 = uniform(0.0, TWOPI)
    x0 = uniform(0.0, 1.0)
    r1 = sqrt(1.0 - x0)
    x = sin(t1) * r1
    y = cos(t1) * r1
    t2 = uniform(0.0, TWOPI)
    r2 = sqrt(x0)
    z = sin(t2) * r2
    w = cos(t2) * r2
    return x, y, z, w
...
def quatToAxisAngle(x, y, z, w):
    ...
    """
    if w >= 1.0 or w <= -1.0:
        return 1.0, 0.0, 0.0, 0.0  # axis angle identity

    angle = 2.0 * acos(w)
    inv_sin_half_angle = 1.0 / sin(angle / 2.0)
...
def qinv(x, y, z, w):
    ...
    """
    inv_squared_magnitude = 1.0 / (x * x + y * y + z * z + w * w)
