...
class GridGUITest(unittest.TestCase):
    ...
    def test_loadReceptorPDB(self):
        ...
        assert set(rec.getData("category")) == set(
            ["_wate", "cmpsc", "missa", "other", "prote", "modif"]
        )
        ...
        assert set(
            [str(root.child(n).text(0)) for n in range(root.childCount())]
        ) == set(["A", "B"])
        AItem = root.child(0)
        ...
        altcats = [
            str(altCatItem.child(n).text(0)) for n in range(altCatItem.childCount())
        ]
        assert set(altcats) == set(
            ["protein", "sidechains to complete", "missing atoms", "water"]
        )
        ...
        ile204 = [
            ...
            if str(item.child(n).text(0)) == "ILE'204"
        ][0]
        ...
        assert set(ser10.getNames()) == set(
            ["N", "CA", "C", "O", "CB", "OG", "H", "H", "H", "H"]
        )
