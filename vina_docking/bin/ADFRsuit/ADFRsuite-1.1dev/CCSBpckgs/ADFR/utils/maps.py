...
class MapsFile:
    ...
    def printInfo(self, indent=''):
        ...
        for at in d['mapTypes']:
            ...
            n +=1
        print
        if d.has_key('wMapWeight'):
            print indent+'    W map    : weight %.2f entropy %.2f'%(d['wMapWeight'],d['wMapEntropy'])
        if d.has_key('mapGradients') and d['mapGradients']:
            ...
            for res in residues.replace(',', ' ').split():
                ...
                try:
                    ...
                    if not len(resnumStr) or not icode.isalpha():
                        print 'ERROR: invalid syntax in residue %s.\n Expected a residue number, got "%s\n"'% (res, res[3:])
                        continue
                    resnum = int(resnumStr)
                fr.append( (resname.upper(), resnum))#, icode) )           
            flexRes.append( [chid, fr] )
    else:
        flexRes = []
    return flexRes
#from ADFR.utils.maps import parseResString
#parseResString("A:GLU_2_, ASN_3, PHE_4_A")

def parseResString(resStr):
    # "A:GLU_2_, ASN_3, PHE_4_A;..."  --> [['A', [('GLU', 2, "_"), ('ASN', 3, "_"), ('PHE', 4, 'A')]]]
    ...
    for expr in resStr.split(';'):
        ...
        for res in residues.replace(',', ' ').split():
            ...
            if len(items) == 2:
                ...
                print "ERROR: unsupported format for residues string %s. \nFormat: Chid:Resname_Resnum_Icode,...;Chid:Resname_Resnum.\nNote: Icode is optional.\n Example: %s" % (resStr, "A:GLU_2_,ASN_3,PHE_4_A;...")
                return []
            _reslist.append((resname, int(resnum), icode))
        reslist.append([chid, _reslist])
        #
    return reslist
