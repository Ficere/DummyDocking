...
class CalculateAD4Grids:
    ...
    def run(self, background=False):
        ...
        if self.platform == "Darwin":
            ...
            lydPath = os.getenv("DYLD_LIBRARY_PATH")
            if not lydPath:
                print ("WARNING: DYLD_LIBRARY_PATH is not set for running %s" % self.binary)
            else:
                #print ("Lydpath: %s" %lydPath)
                command = 'DYLD_LIBRARY_PATH=%s ' % lydPath + command
