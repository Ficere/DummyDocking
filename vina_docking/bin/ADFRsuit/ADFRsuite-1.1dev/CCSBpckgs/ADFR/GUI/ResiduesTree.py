...
class ResiduesTree(QtGui.QWidget):
    ...
    def buildUI(self):
                ...
                if rnames[0] == rnames[1]:
                    ...
                    if self._resToItem.has_key(resKey):
                        if rnames[0] != "MSE":
                            mut.append(resKey)
            if len(mut):
                self.resCategories["mutated"] = mut
            if len(constr):
                self.resCategories["constructed"] = constr
        ...
        if self._altloc is not None:
            ...
            for chid, reslist in self._altloc.items():
                for resname, resnum, icode in reslist:
                    ...
                    if self._resToItem.has_key(resKey):
                        altloc.append(resKey)
            if len(altloc):
                self.resCategories["alternate locations"] = altloc
        ...
        if len(self.resCategories):
            ...
            for name in ["alternate locations", "mutated", "constructed", "MSE"]:
                ...
                b1.clicked.connect(
                    CallbackFunction(self._toggleSelectResCategory, name, True)
                )
                b2.clicked.connect(
                    CallbackFunction(self._toggleSelectResCategory, name, False)
                )
    ...
    def buildTree(self):
        ...
        if self._sites:
            siteRootItem = QtGui.QTreeWidgetItem(
                treeWidget.invisibleRootItem(), ["binding sites for:"]
            )
            ...
            for res in chain.iterResidues():
                ...
                if self.checkedItems:
                    if self.checkedItems["%s:%d%s" % (chid, resnum, icode)]:
                        ...
                        if len(icode):
                            sel = res.select(
                                "chain %s resnum `%d` icode %s" % (chid, resnum, icode)
                            )
                        else:
    ...
    def selectClose(self):
        ...
        atoms = findNeighbors(
            self.ligand._ag, self.distanceWidget.value(), self.receptor
        )
        if len(atoms):
            self._suspend = True
            ...
            if self.checkedItems:
                indicesInBox = []
                for ind in indices:
                    if self.checkedItems.has_key(
                        "%s:%d%s" % (chids[ind], resnums[ind], icodes[ind])
                    ):
                        indicesInBox.append(ind)
                indices = indicesInBox
            selStr = "(same residue as index %s) and ca" % " ".join(
    ...
    def getAtoms(self):
        if len(self._CAindices) == 0:
            ...
            return self.receptor.select(
                "same residue as index %s" % " ".join([str(x) for x in self._CAindices])
            )
    ...
    def onClick(self, item, column):
        ...
        if level == 1:
            if str(_item.text(0)).find("binding sites") >= 0:
                ...
                res = (
                    item._pmvObj
                )  # this item is a residue item under a chain tree item(not in sites)
                inds = res.getIndices()
...
class ResiduesTreeDialog(QtGui.QDialog):
    ...
    def __init__(
        ...
        self.resTreeWidget = ResiduesTree(
            app,
            rec,
            lig,
            checkedItems,
            ...
            mutated=mutated,
            mse=mse,
            flexResiduesNames=flexResiduesNames,
        )
