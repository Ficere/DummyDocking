...
class MyTableItem(QtGui.QTableWidgetItem):
    ...
    def __lt__(self, other):
        try:
            return (float(self.data(QtCore.Qt.EditRole)) <
                    float(other.data(QtCore.Qt.EditRole)))
        except ValueError:
...
class PairwiseScoreTable(QtGui.QWidget):
    ...
    def setPrecision(self, precision):
        ...
        self._toosmall = '0.'
        for n in range(precision): self._toosmall+='0'
        #print 'FOGO', self._precision, self._format, self._toosmall
        self.updateTable()
        
    def setEnergyLabel(self, vdw, elec, hb, ds):
        self.energyLabel.setText('ene.: %.2f vdw: %.2f el: %.2f hb: %.2f ds: %.2f '%(
    ...
    def buildUI(self):
        ...
        self.columnLabels = ['num', 'at1', 'at2', 'total', 'vdw', 'elec', 'hb', 'ds', 'dist']
    ...
    def getValueString(self, value):
        ...
        if valstr==self._toosmall or valstr=='+'+self._toosmall or valstr=='-'+self._toosmall:
            valstr = '.'
        return valstr
    ...
    def updateTable(self):
        ...
        ramp = GreenWhiteRed(upper=1.)

        maxi = max( max(self._vdwArray.flat), max(self._eArray.flat),
                    max(self._hArray.flat), max(self._dsArray.flat),)
        mini = min( min(self._vdwArray.flat), min(self._eArray.flat),
                    min(self._hArray.flat), min(self._dsArray.flat),)
        ...
        keptPairs = []
        for i,j in pairs:
            strs = [self.getValueString(tot[n]), self.getValueString(vdw[n]),
                    self.getValueString(elec[n]), self.getValueString(hb[n]),
                    self.getValueString(ds[n])]
            if set(strs)==set(['.']):
                n += 1
                continue # all entries are below precision (i.e. '.'), we skip
            #print k, i, j, strs
            self.table.setItem(k, 0, MyTableItem(str(k))) # hidden colum to find pairs after sort
            self.table.setItem(k, 1, QtGui.QTableWidgetItem(
                '%s`%s:%s'%(resnames1[i],resnums1[i],names1[i])))
            self.table.setItem(k, 2, QtGui.QTableWidgetItem(
