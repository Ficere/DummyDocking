...
class _loop:
    ...
    def addName(self, name):
        ...
        if catName[1]:
            if not self.parserObj.currentTarget[-2].has_key(catName[0]):
                self.parserObj.currentTarget[-2][catName[0]] = {}
            if not self.parserObj.currentTarget[-2][catName[0]].has_key(catName[2]):
...
class CIFparser:
    ...
    def setDataName(self, name):
        ...
        if name[1]:
            if not self.currentTarget[-1].has_key(name[0]):
    ...
    def selectFrame(self, name=""):
        if not self.currentTarget[1].has_key(name):
...
def __loadCIFdic__(dicFile):
    ...
    try:
        ...
        for k, v in parser.data["data_mmcif_pdbx.dic"].iteritems():
            if type(v) != dict or not v.has_key("item_type"):
                continue
            name = partitionString(k[6:], ".")
            if not dic.has_key(name[0]):
                dic[name[0]] = {}
        ...
        for k2, v2 in v.iteritems():
            for k3, v3 in v2.iteritems():
                for i in xrange(len(v3)):
                    v2[k3][i] = not (v3[i] == "?" or v3[i] == ".") and v3[i] or None
