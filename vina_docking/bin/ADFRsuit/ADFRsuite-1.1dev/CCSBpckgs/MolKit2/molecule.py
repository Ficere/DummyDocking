...
#
import prody, os, numpy, re, sys, gzip
...
from .cifInterface import CIFToProdyAG
...
class AssignSecondaryStructureWtihPross:
    ...
    def _rc_find(self, codes, pattern):
        ...
        try:
            while 1:
                mat = it.next()
                matches.append((int(mat.start()/CODE_LENGTH),
                                int(mat.end()/CODE_LENGTH)))
...
indices are 1-based
    ...
    def buildBondsByDistance(self, factor=1.175):
        ...
        #t0 = time()
        #print 'BuildBonds for', self.name
        bo = self.getBondOrdRadius()
        ag = self._ag
        v = ag.getCoords().astype('f')
        noWatInd = self._ag.select('not water').getIndices()
        bht = bhtreelib.BHtree( v[noWatInd], bo[noWatInd], 10)
        pairs = bht.closePointsPairsInTree(1.175)
        del bht
        alts = ag.getAltlocs()
        if alts is not None and len(set(alts)-set([' '])):
            # there are alternate conformation, only allow bonds between
            # ' ' and alternates and alternates with same letter
            ind = []
            for n, p in enumerate(pairs):
                i, j = p
                alti = ag._data['altloc'][i]
                altj = ag._data['altloc'][j]
                if alti==' ' or altj==' ' or alti==altj:
                    ind. append(n)
            ppairs = pairs[ind]
        else:
            ppairs = pairs
...
After running this command, atoms will have:
   ...
   - HB donor acceptor properties ('hbType')
    ...
    def computeDoubleBond(self, a1, a2, d=0.12):
        ...
        # d is the separation between the 2 parallel lines
        ag = self._ag
        a1 = ag[a1]
        a2 = ag[a2]
    ...
    def computeTripleBond(self, a1, a2, d=0.2):
        # compute vertices for a single triple bond by computing
    ...
    def computeTripleBondsVertices(self, bonds, separation):
        ...
        # for bonds flanking the central bond drawn as single bond
        #       x3 ---------- x4
        #       x1 ---------- x2
        #       x5 ---------- x6
        #
...
"""
    ...
    def measureCHIs(self, chain, resnum, resname=None, rotlib=None):
        ...
        self.measureCHIsInRes(resAtoms, resname, rotlib)
        
    def measureCHIsInRes(self, resAtoms, resname=None, rotlib=None):
        if rotlib is None:
        ...
        for angleDef, movedAtoms in chiDef:
            if len(angleDef):
                ...
                if atoms and len(atoms) == 4:                    
                    ...
                    # once an atom is missign in side chain we can stop
                    break
...
class MultiMolecule(Molecule):
    ...
    def __init__(self, filename, fileFormat='auto', group=None, indexingMode='foreground'):
        ...
        assert self.fileFormat in ['mol2','sdf', 'cif', 'cif.gz'], \
               "ERROR: bad fileFormat parameter, got %s expected 'mol2', 'sdf, cif'"%self.fileFormat

        if filename[-3:].lower() == ".gz":
            self._fp = gzip.open(cifFile)
        else:
            self._fp = open(filename)
        self.data = mmap.mmap(self._fp.fileno(), 0, access=mmap.ACCESS_READ)
        self.numberOfMolecules = -1
        self.index = [] # character offset in the file
        self.componentNames = [] # molecule names
        self._hasIndex = False

        self.group = group

        ag, header = self.getFirstAtomgroup()

        Molecule.__init__(self, ag.getTitle(), ag, filename=filename)
        if self.fileFormat in ['cif', 'cif.gz']:
            self.cifHeader = header
    ...
    def getFirstAtomgroup(self):
        ...
        elif self.fileFormat in ['cif', 'cif.gz']:
            end = self.data.find("data_", 5)
            source = self.data[0:end]
            parser = CIFparser()
            parser.parseString(self.data[0:end])
            for name, data in parser.data.items():
                return CIFToProdyAG(name[5:], data)
    ...
    def gotoMolecule(self, idx):
        ...
        if self.fileFormat in ['mol2', 'sdf']:
            ag = self.agFromOBmolSource(source) # prody molecule for current index
        elif self.fileFormat in ['cif', 'cif.gz']:
            parser = CIFparser()
            parser.parseString(source)
            for name, data in parser.data.items():
                ag, header = CIFToProdyAG(name[5:], data)
                self.cifHeader = header
                break
        self.name = ag.getTitle()
        self.setAtomgroup(ag)
        self.currentMoleculeIndex = idx

    def gotoNext(self):
        if self.currentMoleculeIndex < len(self.index)-1:
            self.gotoMolecule(self.currentMoleculeIndex+1)
            return True
        else:
            return False
        
    def gotoPrevious(self):
        if self.currentMoleculeIndex > 0:
            self.gotoMolecule(self.currentMoleculeIndex-1)
            return True
        else:
            return False
        
    def getMoleculeIndexByName(self, string):
        for n, name in enumerate(self.componentNames):
    ...
    def buildIndex(self):
        ...
        if os.path.exists(indexFilename):
            ...
            if abs(os.path.getmtime(self.filename)-modeTimeFromCache)>0.01:
                ...
                self.index = [int(x) for x in lines[:nb]]
                self.componentNames = [x for x in lines[nb:2*nb]]
                self._hasIndex = True
        else:
            buildIndex = True
        ...
        if buildIndex: # build cache
            ...
            if self.fileFormat=='mol2':
                ...
                while True:
                    ...
                    name = self.data[start+18: self.data.find("\n",start+18)]
                ...
                ct = 0
                self.index = [0]
                name = self.data[0: self.data.find("\n", 0)]
                self.componentNames.append(name)
                while True:
                    end = self.data.find("$$$$\n", start)
                    if end==-1: break
                    self.index.append(end+5)
                    name = self.data[end+5: self.data.find("\n", end+5)]
                    self.componentNames.append(name)
                    start = end+5
                    ct += 1
                    if ct%1000==0:
                ...
                # each molecule starts with data_
                ct = 0
                while True:
                    start = self.data.find("data_", start)
                    if start == -1: break
                    self.index.append(start)
                    name = self.data[start+5: self.data.find("\n",start+5)]
                    self.componentNames.append(name)
                    start += 5
                    ct += 1
                    if ct%1000==0:
                ...
                return
            self._hasIndex = True
            print 'MSG: indexed %d compounds in %.3f'%(len(self.index), time()-t0)
            ...
            f.write('%d\n'%len(self.index)) # write the number of components
            assert len(self.index)==len(self.componentNames)
            dum = [f.write('%i '%x) for x in self.index]
            dum = [f.write('%s '%x) for x in self.componentNames]
            f.close()
        else:
            self._hasIndex = True
