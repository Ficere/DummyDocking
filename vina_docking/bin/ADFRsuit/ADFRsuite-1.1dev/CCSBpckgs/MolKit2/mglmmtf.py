...
def MMTFtoPrody(decoder, name="NoName"):
    ...
    for modelIndex, modelChainCount in enumerate(decoder.chains_per_model):
        ...
        groupOff = 0
        for chainIndex in xrange(
            modelChainCount
        ):  # decoder.chains_per_model[modelIndex]):
            ...
            for groupIndex in xrange(nbGroupsInChain):
                ...
                resnums[atomIndex : atomIndex + nbAtoms] = decoder.group_id_list[
                    groupOff + groupIndex
                ]
                resnames[atomIndex : atomIndex + nbAtoms] = group["groupName"]
                # import pdb; pdb.set_trace()
                bonds.extend(
                    (np.array(group["bondAtomList"], "i") + atomIndex)
                    .reshape(-1, 2)
                    .tolist()
                )
