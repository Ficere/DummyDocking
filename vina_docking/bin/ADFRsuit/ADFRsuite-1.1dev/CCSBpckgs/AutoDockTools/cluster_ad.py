...
class Clust(UserList.UserList):
    def __init__(self, mol, seed, coords):
        ...
        self.min_energy = self.max_energy = mol.autodock_states[
            seed
        ].e_binding  ##### get seed energy HERE
        self.append(seed)
...
class Cluster_AD:
    ...
    def __init__(self, mol):
        ...
        self.rmsdarray = (
            numpy.zeros([len(self.mol.autodock_states), len(self.mol.autodock_states)])
            - 1.0
        )
    ...
    def get_coords(self, mol, val):
        ...
        if val != -1:
            ...
            for k in range(state.ntorsions):
                ...
                SetTors.doit(at1, at2, angList[k], thisT.mov_atoms)
            SetQuat.doit(allAtoms, state.quaternion, state.origin, state.translation)
    ...
    def make_clusters(self, tolerance=None, ref=None):
        ...
        for i in range(len(self.argsort)):
            ...
            if probe_conf_ix == self.reference:
                continue
