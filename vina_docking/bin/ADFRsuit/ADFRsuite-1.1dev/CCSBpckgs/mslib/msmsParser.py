...
class MSMSParser:
    ...
    def getFaces(self, faceDescr):
        ...
        self.faces = numpy.array(
            map(lambda x: [int(x[0]) - 1, int(x[1]) - 1, int(x[2]) - 1], facesInfo)
        ).astype("i")
        self.facesProp = numpy.array(
            map(lambda x: [int(x[3]), int(x[4])], facesInfo)
        ).astype("i")
    ...
    def getVert(self, vertDescr):
        ...
        self.vertices = numpy.array(
            map(lambda x: [float(x[0]), float(x[1]), float(x[2])], vertInfo)
        ).astype("f")
        self.normals = numpy.array(
            map(lambda x: [float(x[3]), float(x[4]), float(x[5])], vertInfo)
        ).astype("f")
        self.vertProp = numpy.array(
            map(lambda x: [int(x[6]), int(x[7]), int(x[8])], vertInfo)
        ).astype("i")
